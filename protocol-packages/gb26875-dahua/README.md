...
- 接口为 `go-protocol-v2`，能力为 decode、ingress、encode，网络 TCP_UDP。默认 stdin 一个 JSON 请求、stdout 一个 JSON 结果后退出；`IOT_PROTOCOL_WORKER_MODE=serve` 时逐行处理请求并带回 `requestId`，供平台常驻复用。
...
新设备在「设备管理 → 添加设备」中选择或新建共享监听，接入点在「设备模板 → 详情 → 接入点」维护；已定义命令在「设备管理 → 详情 → 设备控制」执行；「模拟设备测试」用于测试报文。平台操作入口不改变本 module 的 Worker 契约。平台账号需要对应菜单、操作及设备范围授权；详细步骤由上面的平台文档维护。
