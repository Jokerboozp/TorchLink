...
async function generate() {
  if (!file.value && !(form.inputKind==='point-table'?form.pointTable.trim():form.samplePayload.trim())) return UiMessage.warning('请上传文件或填写内容')
  if (file.value?.size>32*1024*1024) return UiMessage.warning('文件不能超过 32 MiB')
...
<template>
  <div class="protocol-generator" :aria-busy="!!busy">
    ...
    <ui-form v-if="step==='input'" label-position="top" :disabled="!!busy" @submit.prevent="generate">
      ...
      <ui-radio-group v-model="form.inputKind" @change="changeKind" class="bottom-gap segmented-choice-group" aria-label="上传类型"><ui-radio-button value="sample">报文</ui-radio-button><ui-radio-button value="point-table">点表</ui-radio-button></ui-radio-group>
      <ui-form-item :label="form.inputKind==='point-table'?'上传点表':'上传报文'"><FilePicker :key="form.inputKind" :accept="form.inputKind==='point-table'?'.xlsx,.csv':'.json,.txt,.hex,.bin'" @change="chooseFile" /><small class="subline">{{ file?.name || (form.inputKind==='point-table'?'Excel / CSV':'JSON / TXT / HEX / BIN') }}</small></ui-form-item>
      <ui-form-item v-if="form.inputKind==='point-table'" label="或粘贴 CSV 点表"><ui-input v-model="form.pointTable" type="textarea" :rows="5" placeholder="name,functionCode,address,addressNotation,dataType&#10;temperature,3,0,zero_based,uint16" /></ui-form-item>
      <ui-form-item v-else label="或粘贴报文"><ui-input v-model="form.samplePayload" type="textarea" :rows="5" placeholder='{"data":{"temperature":25.5,"smoke":false}}' /></ui-form-item>
      </section>
      <section class="generator-section"><div class="generator-section-heading"><h3>协议基本信息</h3><p>确认协议名称、传输方式和样本解析格式。</p></div>
      <div class="form-grid"><ui-form-item label="协议名称"><ui-input v-model="form.name" placeholder="可由文件名生成" /></ui-form-item><ui-form-item label="传输方式"><ui-select v-model="form.transport"><ui-option v-for="item in transports" :key="item" :label="transportLabel(item)" :value="item" /></ui-select></ui-form-item></div>
      ...
      <div class="generator-actions"><ui-button v-permission="'POST /api/v1/ai/protocol-assistant/generate'" type="primary" :loading="busy==='generate'" native-type="submit">生成协议</ui-button></div>
    </ui-form>
    <template v-else>
      <div class="section-toolbar"><strong>{{ form.name }} <small class="muted-text">{{ parsers[draft.parserType] || draft.parserType }}</small></strong><ui-button v-if="!saved" :disabled="!!busy" @click="step='input'">返回修改</ui-button><ui-button v-if="saved" :disabled="!!busy" @click="newVersion">新建版本</ui-button><ui-tag v-if="saved" :type="saved.status==='PUBLISHED'?'success':'info'">{{ {DRAFT:'草稿',VALIDATED:'已校验',PUBLISHED:'已发布'}[saved.status] || saved.status }}</ui-tag></div>
      ...
      <ui-table v-if="saved || !supported" :data="fields" max-height="230" size="small"><ui-table-column prop="name" label="字段" min-width="140" /><ui-table-column prop="address" label="路径 / 地址" min-width="190" /><ui-table-column prop="type" label="类型" min-width="100" /></ui-table>
      <template v-if="supported">
        <ui-form label-position="top" class="top-gap" :disabled="!!busy">
          <div class="form-grid"><ui-form-item label="协议标识"><ui-input v-model="form.protocol" :disabled="!!saved" /></ui-form-item><ui-form-item label="版本"><ui-input v-model="form.version" :disabled="!!saved" /></ui-form-item></div>
