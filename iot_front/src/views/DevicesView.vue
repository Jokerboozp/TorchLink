...
<template>
  ...
  <ui-card shadow="never" class="surface-card table-card"> <!-- 渲染 ui-card 界面元素。 -->
    <ui-table v-loading="loading" :data="registry" stripe> <!-- 渲染 ui-table 界面元素。 -->
      ...
      <ui-table-column label="启用状态" width="105"><template #default="{ row }"><ui-tag :type="tagType(row.device.status)" round>{{ label(enabledStatuses, row.device.status) }}</ui-tag></template></ui-table-column> <!-- 渲染 ui-table-column 界面元素。 -->
      <ui-table-column label="设备角色" width="120"><template #default="{ row }"><ui-tag round>{{ label(deviceRoles, roleOf(row.device), '直接设备') }}</ui-tag><small v-if="row.device.autoRegistered" class="subline">网关自动注册</small></template></ui-table-column> <!-- 渲染 ui-table-column 界面元素。 -->
