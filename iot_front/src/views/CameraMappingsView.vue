...
async function save() { /* 定义 save 函数。 */
  try { /* 执行当前语句并推进处理流程。 */
    ...
    ElMessage.success('摄像头信息已保存') /* 执行当前语句并推进处理流程。 */
...
function consumeNavigationAction() { /* 定义 consumeNavigationAction 函数。 */
  ...
  try { /* 执行当前语句并推进处理流程。 */
    ...
    if (!target) return ElMessage.warning(`当前列表未找到摄像头 ${detail.cameraId}`) /* 判断条件并选择处理分支。 */
    highlightedCameraId.value = target.cameraId /* 更新 highlightedCameraId.value 的值。 */
    ElMessage.info(`已定位摄像头：${target.cameraName || target.cameraId}。直播流由外部视频平台提供。`) /* 执行当前语句并推进处理流程。 */
...
<template>
  <div class="page-toolbar"> <!-- 渲染 div 界面元素。 -->
    <el-button v-permission="'POST /api/v1/integrations/video/cameras'" type="primary" @click="open()">新增摄像头</el-button> <!-- 渲染 el-button 界面元素。 -->
    <el-button @click="load">刷新</el-button> <!-- 渲染 el-button 界面元素。 -->
    <span>共 {{ total }} 个摄像头；一个摄像头最多关联一个设备，一个设备可以关联多个摄像头</span> <!-- 渲染 span 界面元素。 -->
  </div> <!-- 结束当前界面区域。 -->

  <el-alert title="平台只保存摄像头基础信息和设备关联，不解析、拉取或预览视频流。设备告警会带出关联摄像头信息，直播流请由外部视频平台按摄像头信息提供。" type="info" :closable="false" show-icon /> <!-- 渲染 el-alert 界面元素。 -->

  <el-card shadow="never" class="surface-card table-card"> <!-- 渲染 el-card 界面元素。 -->
    <el-table v-loading="loading" :data="cameras" stripe :row-class-name="rowClassName"> <!-- 渲染 el-table 界面元素。 -->
      <el-table-column label="摄像头" min-width="180"><template #default="{ row }"><b>{{ row.cameraName }}</b><small class="subline">{{ row.cameraId }} · {{ row.brand || '品牌未填' }}</small></template></el-table-column> <!-- 渲染 el-table-column 界面元素。 -->
      <el-table-column label="摄像头点位" min-width="170"><template #default="{ row }">{{ row.cameraPoint || '—' }}</template></el-table-column> <!-- 渲染 el-table-column 界面元素。 -->
      <el-table-column label="位置" min-width="210"><template #default="{ row }">{{ [row.building, row.floor, row.room].filter(Boolean).join(' / ') || '—' }}</template></el-table-column> <!-- 渲染 el-table-column 界面元素。 -->
      <el-table-column label="关联设备" min-width="180"><template #default="{ row }">{{ row.deviceId || '未关联' }}</template></el-table-column> <!-- 渲染 el-table-column 界面元素。 -->
      <el-table-column label="状态" width="100" align="center"><template #default="{ row }"><el-tag :type="row.enabled ? 'success' : 'info'" round>{{ row.enabled ? '已启用' : '已停用' }}</el-tag></template></el-table-column> <!-- 渲染 el-table-column 界面元素。 -->
      <el-table-column label="操作" width="100" align="center" fixed="right"><template #default="{ row }"><el-button v-permission="'PUT /api/v1/integrations/video/cameras/:id'" plain type="primary" @click="open(row)">编辑</el-button></template></el-table-column> <!-- 渲染 el-table-column 界面元素。 -->
  ...
  <el-dialog v-model="dialogVisible" :title="editing ? '编辑摄像头信息' : '新增摄像头信息'" width="min(680px, 94vw)"> <!-- 渲染 el-dialog 界面元素。 -->

    <el-form :model="camera" label-position="top"> <!-- 渲染 el-form 界面元素。 -->
      <div class="form-grid"> <!-- 渲染 div 界面元素。 -->
        <el-form-item label="摄像头标识"><el-input v-model="camera.cameraId" :disabled="!!editing" placeholder="外部视频平台摄像头标识" /></el-form-item> <!-- 渲染 el-form-item 界面元素。 -->
        <el-form-item label="品牌"><el-input v-model="camera.brand" placeholder="例如：海康、大华" /></el-form-item> <!-- 渲染 el-form-item 界面元素。 -->
        <el-form-item label="摄像头名称"><el-input v-model="camera.cameraName" placeholder="例如：一层大厅东侧" /></el-form-item> <!-- 渲染 el-form-item 界面元素。 -->
        <el-form-item label="摄像头点位"><el-input v-model="camera.cameraPoint" placeholder="例如：东侧入口" /></el-form-item> <!-- 渲染 el-form-item 界面元素。 -->
        <el-form-item label="建筑"><el-input v-model="camera.building" /></el-form-item> <!-- 渲染 el-form-item 界面元素。 -->
        <el-form-item label="楼层"><el-input v-model="camera.floor" /></el-form-item> <!-- 渲染 el-form-item 界面元素。 -->
        <el-form-item label="房间"><el-input v-model="camera.room" /></el-form-item> <!-- 渲染 el-form-item 界面元素。 -->
        <el-form-item label="关联设备（可选）"><el-select v-model="camera.deviceId" clearable filterable placeholder="选择一个设备"><el-option v-for="item in devices" :key="item.id" :label="`${item.name || item.id} · ${item.id}`" :value="item.id" /></el-select></el-form-item> <!-- 渲染 el-form-item 界面元素。 -->
      </div> <!-- 结束当前界面区域。 -->
      <el-alert title="保存后只能保留一个设备关联；同一设备可以在多个摄像头记录中出现。直播地址、开发工具包、流媒体服务均不在此配置。" type="info" :closable="false" show-icon /> <!-- 渲染 el-alert 界面元素。 -->
      <el-form-item><el-switch v-model="camera.enabled" active-text="启用该摄像头" /></el-form-item> <!-- 渲染 el-form-item 界面元素。 -->
    </el-form> <!-- 结束当前界面区域。 -->
