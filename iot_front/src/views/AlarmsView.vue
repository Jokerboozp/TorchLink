...
async function pollAnalysis(jobId, alarmId, viewToken = analysisViewToken) { /* 定义 pollAnalysis 函数。 */
  ...
  try { /* 执行当前语句并推进处理流程。 */
    ...
    if (progress.status === 'succeeded') { /* 判断条件并选择处理分支。 */
      ...
      analysisLoading.value = false /* 更新 analysisLoading.value 的值。 */
      UiMessage.success('智能研判已完成') /* 执行当前语句并推进处理流程。 */
      return /* 返回当前处理结果。 */
    } /* 结束当前表达式或代码块。 */
...
async function action(id, value) { /* 定义 action 函数。 */
  ...
  try { /* 执行当前语句并推进处理流程。 */
    ...
    UiMessage.success('操作成功') /* 执行当前语句并推进处理流程。 */
    await load() /* 等待异步操作完成。 */
  } catch (e) { /* 结束当前表达式或代码块。 */
    notifyError(e) /* 执行当前语句并推进处理流程。 */
  } finally { /* 结束当前表达式或代码块。 */
...
<template>
  ...
  <ui-dialog v-model="detailVisible" title="告警详情" width="min(760px, 94vw)" @closed="handleDetailClosed"> <!-- 渲染 ui-dialog 界面元素。 -->
    <ui-descriptions v-if="detail" :column="1" border> <!-- 渲染 ui-descriptions 界面元素。 -->
      <ui-descriptions-item label="告警编号">{{detail.alarmId}}</ui-descriptions-item><ui-descriptions-item label="设备">{{detail.deviceName||detail.deviceId}}</ui-descriptions-item><ui-descriptions-item v-if="detail.componentId" label="部件">{{detail.componentName||detail.componentId}}（{{detail.componentId}}）</ui-descriptions-item><ui-descriptions-item v-if="detail.componentLocation" label="部件位置">{{detail.componentLocation}}</ui-descriptions-item><ui-descriptions-item label="告警类型">{{alarmType(detail.alarmType)}}</ui-descriptions-item><ui-descriptions-item label="等级 / 状态"><ui-tag :type="tagType(detail.alarmLevel)">{{label(alarmLevels,detail.alarmLevel)}}</ui-tag> {{label(alarmStatuses,detail.status)}}</ui-descriptions-item><ui-descriptions-item label="来源">{{label(alarmSources,detail.source,'其他来源')}}</ui-descriptions-item><ui-descriptions-item label="首次发生">{{formatTime(detail.firstTriggeredAt)}}</ui-descriptions-item><ui-descriptions-item label="最后发生">{{formatTime(detail.lastTriggeredAt)}}</ui-descriptions-item><ui-descriptions-item label="触发次数">{{detail.triggerCount}}</ui-descriptions-item> <!-- 渲染 ui-descriptions-item 界面元素。 -->
    </ui-descriptions> <!-- 结束当前界面区域。 -->
    <ui-card shadow="never" class="top-gap"> <!-- 渲染 ui-card 界面元素。 -->
      ...
      <div v-if="analysisProgress" class="analysis-progress" aria-live="polite"> <!-- 渲染 div 界面元素。 -->
        ...
        <ui-progress :percentage="progressPercent" :status="progressStatus" :stroke-width="10" /> <!-- 渲染 ui-progress 界面元素。 -->
