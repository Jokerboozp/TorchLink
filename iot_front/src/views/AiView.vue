...
const controlsExpanded = ref(false) /* 声明 controlsExpanded。 */
...
function startCreateAgent() { /* 定义 startCreateAgent 函数。 */
  ...
  ElMessage.info('已打开新建智能体，请填写配置清单结构化数据') /* 执行当前语句并推进处理流程。 */
} /* 结束当前表达式或代码块。 */
...
async function saveAgent() { /* 定义 saveAgent 函数。 */
  ...
  catch { ElMessage.error('智能体结构化数据格式不正确'); return } /* 执行当前语句并推进处理流程。 */
  if (editingAgentId.value && manifest?.id !== editingAgentId.value) { /* 判断条件并选择处理分支。 */
    ElMessage.error('编辑时不能修改智能体的唯一标识；如需新插件请先新建') /* 执行当前语句并推进处理流程。 */
    return /* 返回当前处理结果。 */
  } /* 结束当前表达式或代码块。 */
  ...
  try { /* 执行当前语句并推进处理流程。 */
    ...
    agentEditorVisible.value = false /* 更新 agentEditorVisible.value 的值。 */
    ElMessage.success(`${editing ? '智能体已更新' : '智能体已创建'}：${workflowName(saved)}`) /* 执行当前语句并推进处理流程。 */
...
async function toggleWorkflow(item) { /* 定义 toggleWorkflow 函数。 */
  ...
  try { /* 执行当前语句并推进处理流程。 */
    ...
    ElMessage.success(`${workflowName(item)}已${manifest.enabled ? '启用' : '禁用'}`) /* 执行当前语句并推进处理流程。 */
...
async function deleteWorkflow(item) { /* 定义 deleteWorkflow 函数。 */
  ...
  try { /* 执行当前语句并推进处理流程。 */
    await ElMessageBox.confirm(`删除后将无法运行“${workflowName(item)}”，确定继续吗？`, '删除工作流插件', { type:'warning', confirmButtonText:'确定删除', cancelButtonText:'取消' }) /* 等待异步操作完成。 */
  ...
  try { /* 执行当前语句并推进处理流程。 */
    ...
    ElMessage.success(`已删除工作流插件：${workflowName(item)}`) /* 执行当前语句并推进处理流程。 */
...
<template>
    ...
    <el-card shadow="never" class="surface-card control-card"> <!-- 渲染 el-card 界面元素。 -->
      ...
      <div class="control-scroll"> <!-- 渲染 div 界面元素。 -->
        ...
        <div class="provider-summary"> <!-- 渲染 div 界面元素。 -->
          ...
          <el-button v-permission="'menu:aiProviders'" type="primary" plain @click="emit('navigate', 'aiProviders')">管理模型服务</el-button> <!-- 渲染 el-button 界面元素。 -->
        </div> <!-- 结束当前界面区域。 -->
        <div class="control-section-label"><span>02</span>选择工作流</div> <!-- 渲染 div 界面元素。 -->
        <el-form label-position="top"><el-form-item label="工作流插件"><el-select v-model="selectedWorkflowId" placeholder="选择智能助手" :disabled="sending || !workflowItems.length"><el-option v-for="item in workflowItems" :key="workflowKey(item)" :label="workflowName(item)" :value="workflowKey(item)" /></el-select></el-form-item></el-form> <!-- 渲染 el-form 界面元素。 -->
        <el-empty v-if="!runtimeLoading && !workflowItems.length" description="暂无可用工作流" :image-size="62" /> <!-- 渲染 el-empty 界面元素。 -->
        <div v-if="selectedWorkflow" class="workflow-description"><div><span class="workflow-icon">流程</span><div><strong>{{ workflowName(selectedWorkflow) }}</strong><small>{{ selectedWorkflow.version ? `v${selectedWorkflow.version}` : '服务端托管' }}</small></div></div><p>{{ selectedWorkflow.description || '该工作流会按服务端策略调用受控工具。' }}</p><div v-if="selectedCapabilities.length" class="capability-list"><el-tag v-for="item in selectedCapabilities" :key="capabilityLabel(item)" size="small" effect="plain">{{ capabilityLabel(item) }}</el-tag></div></div> <!-- 渲染 div 界面元素。 -->
        <div class="control-section-label"><span>03</span>运行参数</div> <!-- 渲染 div 界面元素。 -->
        <el-form class="run-config" label-position="top" :model="runConfig" :disabled="sending"><div><el-form-item label="运行模型"><el-input v-model="runConfig.model" readonly placeholder="使用活动模型" /></el-form-item><el-form-item label="最大输出"><el-input-number v-model="runConfig.maxTokens" :min="128" :max="8192" :step="128" controls-position="right" /></el-form-item></div></el-form> <!-- 渲染 el-form 界面元素。 -->
        ...
        <el-alert v-if="runtimeError" class="runtime-warning" :title="runtimeError" type="warning" :closable="false" show-icon /> <!-- 渲染 el-alert 界面元素。 -->
      </div> <!-- 结束当前界面区域。 -->
    </el-card> <!-- 结束当前界面区域。 -->

    <el-card shadow="never" class="surface-card chat-card ai-chat-card"> <!-- 渲染 el-card 界面元素。 -->
      <template #header><div class="card-header chat-header"><div><strong>{{ workflowName(selectedWorkflow) }}</strong><small>{{ conversationId ? `会话 · ${conversationId}` : '新会话 · 服务端受控租户上下文' }}</small></div><div><el-button plain :disabled="!runs.length" @click="openTrace(runs[0])">运行轨迹</el-button><el-button plain type="warning" :disabled="sending" @click="clearConversation">清空对话</el-button></div></div></template>
  ...
  <el-drawer v-model="managementVisible" title="智能体管理" size="min(760px, 94vw)" class="workflow-manager" append-to-body> <!-- 渲染 el-drawer 界面元素。 -->
    <section class="manager-panel panel-agent"> <!-- 渲染 section 界面元素。 -->
        <div class="manager-intro"><span>智能体管理</span><div><h3>智能体插件管理</h3><el-tag size="small" type="primary" effect="plain">管理员</el-tag></div><p>内置智能体仅可查看；动态智能体可编辑、启用/禁用或删除。点击“新建智能体”即可在弹窗中提交新的配置清单，保存后智能体会立即进入工作流列表。</p></div> <!-- 渲染 div 界面元素。 -->
        <el-alert v-if="!isAdmin" title="工作流插件管理仅限管理员。" type="warning" :closable="false" show-icon /> <!-- 渲染 el-alert 界面元素。 -->
        <div v-else class="workflow-admin-panel"> <!-- 渲染 div 界面元素。 -->
          <div class="workflow-admin-toolbar"><div><strong>已配置的工作流插件</strong><small>{{ workflowManageTotal }} 个聊天插件 · 内置聊天智能体只读；告警研判、设备巡检和协议接入由业务页面调用</small></div><div><el-button size="small" :loading="workflowManageLoading" @click="loadWorkflowManagement">刷新清单</el-button><el-button v-permission="'POST /api/v1/ai/workflows'" size="small" type="primary" plain @click="startCreateAgent">新建智能体</el-button></div></div> <!-- 渲染 div 界面元素。 -->
          <el-alert v-if="workflowManageError" :title="workflowManageError" type="error" :closable="false" show-icon /> <!-- 渲染 el-alert 界面元素。 -->
          <el-skeleton v-if="workflowManageLoading && !workflowManageItems.length" :rows="4" animated /> <!-- 渲染 el-skeleton 界面元素。 -->
          <el-empty v-else-if="!workflowManageItems.length" description="暂无工作流插件" :image-size="56" /> <!-- 渲染 el-empty 界面元素。 -->
          <div v-else class="workflow-admin-list"> <!-- 渲染 div 界面元素。 -->
            <div v-for="item in workflowManageItems" :key="workflowKey(item)" class="workflow-admin-item"> <!-- 渲染 div 界面元素。 -->
              <div class="workflow-admin-main"><div><strong>{{ workflowName(item) }}</strong><el-tag size="small" :type="item.enabled === false ? 'info' : 'success'" effect="plain">{{ item.enabled === false ? '已禁用' : '已启用' }}</el-tag><el-tag v-if="isBuiltinWorkflow(item)" size="small" effect="plain">内置只读</el-tag></div><small>{{ workflowKey(item) }} · {{ item.version ? `v${item.version}` : '无版本' }}</small><p>{{ item.description || '未填写插件说明' }}</p></div> <!-- 渲染 div 界面元素。 -->
          ...
          <div v-if="workflowManageTotal" class="list-pagination"> <!-- 渲染 div 界面元素。 -->
            <el-pagination v-model:current-page="workflowManagePage" v-model:page-size="workflowManagePageSize" :total="workflowManageTotal" :page-sizes="[20, 50, 100]" layout="total, sizes, prev, pager, next, jumper" @current-change="changeWorkflowManagePage" @size-change="changeWorkflowManagePageSize" /> <!-- 渲染 el-pagination 界面元素。 -->
          </div> <!-- 结束当前界面区域。 -->
        </div> <!-- 结束当前界面区域。 -->
      </section> <!-- 结束当前界面区域。 -->
  </el-drawer> <!-- 结束当前界面区域。 -->
  <el-dialog v-model="agentEditorVisible" :title="editingAgentId ? '编辑智能体' : '新建智能体'" width="min(820px, 94vw)" class="agent-editor-dialog" append-to-body destroy-on-close> <!-- 渲染 el-dialog 界面元素。 -->
    <el-alert title="结构化数据标准不支持注释。内置智能体仅可查看，动态智能体可在管理清单中编辑、启用或删除。" type="info" :closable="false" show-icon /> <!-- 渲染 el-alert 界面元素。 -->
    <el-form class="drawer-form" label-position="top" :disabled="!isAdmin || creatingAgent"> <!-- 渲染 el-form 界面元素。 -->
      <el-form-item label="智能体配置清单"><el-input ref="agentEditorRef" v-model="agentJson" class="agent-json-editor" type="textarea" :rows="18" resize="vertical" spellcheck="false" /></el-form-item> <!-- 渲染 el-form-item 界面元素。 -->
      <div class="manifest-guide"> <!-- 渲染 div 界面元素。 -->
        <div class="manifest-guide-title"><div><strong>字段说明</strong><small>所有字段均为必填，请参考下方说明填写。</small></div><el-tag size="small" effect="plain">11 个字段</el-tag></div> <!-- 渲染 div 界面元素。 -->
      ...
      <el-alert title="只允许受控查询工具与“仅生成、不保存”的规则草稿工具；内置智能体不能覆盖，智能体不能直接启用规则。" type="info" :closable="false" show-icon /> <!-- 渲染 el-alert 界面元素。 -->
    </el-form> <!-- 结束当前界面区域。 -->
  ...
  <el-dialog v-model="agentPreviewVisible" title="查看内置智能体配置清单（只读）" width="min(760px, 92vw)" append-to-body> <!-- 渲染 el-dialog 界面元素。 -->
    <el-alert title="内置智能体仅供查看，不能编辑、启用/禁用或删除。" type="info" :closable="false" show-icon /> <!-- 渲染 el-alert 界面元素。 -->
    <div v-if="agentPreview" class="agent-preview-summary"><div><strong>{{ workflowName(agentPreview) }}</strong><el-tag size="small" effect="plain">内置只读</el-tag></div><small>{{ workflowKey(agentPreview) }} · {{ agentPreview.version ? `v${agentPreview.version}` : '无版本' }} · {{ agentPreview.defaultModel || '未设置模型' }}</small><p>{{ agentPreview.description || '未填写插件说明' }}</p></div> <!-- 渲染 div 界面元素。 -->
...
.ai-runtime { flex:none; min-height:74px; margin-bottom:16px; padding:15px 18px; display:flex; align-items:center; justify-content:space-between; gap:20px; background:#fff; border:1px solid #e8e8e8; border-left:3px solid #1677ff; border-radius:4px; }.ai-runtime>div:first-child { display:grid; gap:3px; }.section-kicker { color:#1677ff; font-size:12px; font-weight:700; letter-spacing:.14em; }.ai-runtime strong { font-size:16px; }.ai-runtime small { color:var(--muted); }.runtime-actions { display:flex; align-items:center; gap:12px; }.runtime-status { display:flex; align-items:center; gap:8px; color:#646c73; font-size:13px; white-space:nowrap; }.runtime-status i { width:7px; height:7px; background:#ff4d4f; border-radius:50%; }.runtime-status i.online { background:#52c41a; } /* 定义当前元素的样式规则。 */
.ai-workbench { flex:1; min-height:0; overflow:hidden; display:grid; grid-template-rows:minmax(0,1fr); grid-template-columns:minmax(280px,320px) minmax(0,1fr); gap:16px; align-items:stretch; }.control-card,.ai-chat-card { height:100%; min-height:0; display:flex; flex-direction:column; }.control-card :deep(.el-card__header),.ai-chat-card :deep(.el-card__header) { flex:none; }.control-card :deep(.el-card__body),.ai-chat-card :deep(.el-card__body) { flex:1; min-height:0; overflow:hidden; }.card-header>div { display:grid; gap:3px; }.card-header small { display:block; }.control-card :deep(.el-card__body) { display:flex; padding:0; }.control-scroll { flex:1; min-width:0; min-height:0; padding:16px; overflow:auto; overscroll-behavior:contain; }.control-scroll>.el-alert { margin-bottom:14px; }.workflow-description { margin:-3px 0 14px; padding:12px; background:#f5f9ff; border:1px solid #d6e8ff; border-radius:4px; }.workflow-description>div:first-child { display:flex; align-items:center; gap:9px; }.workflow-description>div:first-child>div { display:grid; gap:2px; }.workflow-description strong { color:#1554ad; font-size:13px; }.workflow-description small { color:#8c8c8c; font-size:12px; }.workflow-description p { margin:9px 0; color:#646c73; font-size:12px; line-height:1.6; }.workflow-icon { width:30px; height:30px; display:grid; place-items:center; color:#fff; background:#1677ff; border-radius:4px; font-size:12px; font-weight:800; }.capability-list { display:flex; flex-wrap:wrap; gap:5px; }.run-config { margin-bottom:2px; }.run-config>div { display:grid; grid-template-columns:minmax(0,1fr) 112px; gap:9px; }.run-config :deep(.el-input-number) { width:100%; }.provider-summary { margin:0 0 14px; padding:11px; display:grid; gap:7px; background:#fafafa; border:1px solid #ededed; border-radius:4px; }.provider-summary>span { color:#8c8c8c; font-size:12px; }.provider-summary strong { font-size:13px; }.provider-summary small { color:#646c73; }.sandbox-collapse { border-top:1px solid #ededed; }.collapse-title { width:100%; padding-right:8px; display:flex; align-items:center; justify-content:space-between; }.collapse-title>div { display:grid; gap:2px; }.collapse-title strong { font-size:13px; }.collapse-title small { color:#8c8c8c; font-size:12px; }.plugin-description { margin:-4px 0 15px; display:grid; gap:4px; }.plugin-description strong { color:#1554ad; font-size:12px; }.plugin-description span { color:#646c73; font-size:12px; line-height:1.5; }.provider-select-row { display:grid; grid-template-columns:minmax(0,1fr) auto; gap:7px; }.provider-select-row :deep(.el-select) { width:100%; }.provider-profile-toolbar { margin:-5px 0 12px; display:flex; align-items:center; justify-content:space-between; gap:8px; }.provider-profile-toolbar small { color:#86909c; font-size:12px; line-height:1.5; }.provider-profile-toolbar>div { display:flex; gap:5px; flex:none; }.provider-profile-editor { margin:0 0 13px; padding:11px; background:#f9f0ff; border:1px solid #d3adf7; border-radius:5px; }.provider-profile-editor :deep(.el-form-item) { margin-bottom:10px; }.provider-profile-actions { display:flex; justify-content:flex-end; gap:7px; margin-top:10px; }.admin-notice { margin-bottom:14px; }.test-button { width:100%; margin-top:12px; }.test-result { margin-top:14px; padding:11px; border:1px solid; border-radius:4px; }.test-result.success { background:#f6ffed; border-color:#b7eb8f; }.test-result.failed { background:#fff2f0; border-color:#ffccc7; }.test-result>div { display:flex; justify-content:space-between; align-items:center; }.test-result p { margin:8px 0; color:#3d3d3d; font-size:12px; line-height:1.6; white-space:pre-wrap; }.test-result small { color:#8c8c8c; word-break:break-all; } /* 定义当前元素的样式规则。 */
.knowledge-summary { margin:0 0 14px; padding:11px; display:grid; gap:5px; background:#f6ffed; border:1px solid #d9f7be; border-radius:4px; }.knowledge-summary>div { display:flex; align-items:center; justify-content:space-between; }.knowledge-summary span,.knowledge-summary small { color:#5b6b59; font-size:12px; }.knowledge-summary strong { color:#237804; font-size:12px; }.binding-numbers { display:grid; grid-template-columns:1fr 1fr; gap:9px; }.binding-numbers :deep(.el-input-number),.sandbox-collapse :deep(.el-select),.sandbox-collapse :deep(.el-radio-group) { width:100%; }.sandbox-collapse :deep(.el-radio-button) { flex:1; }.sandbox-collapse :deep(.el-radio-button__inner) { width:100%; padding-left:7px; padding-right:7px; } /* 定义当前元素的样式规则。 */
...
.agent-preview-summary { margin-top:14px; padding:12px; display:grid; gap:5px; background:#f6faff; border:1px solid #d6e8ff; border-radius:5px; }.agent-preview-summary>div { display:flex; align-items:center; gap:7px; }.agent-preview-summary strong { color:#1f2329; font-size:13px; }.agent-preview-summary small { color:#697386; font-size:12px; }.agent-preview-summary p { margin:0; color:#4e5969; font-size:12px; line-height:1.6; }.agent-manifest-preview { max-height:min(58vh,560px); margin:12px 0 0; padding:14px; overflow:auto; color:#1f2329; background:#fbfcfe; border:1px solid #d6e4ff; border-radius:5px; font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace; font-size:12px; line-height:1.65; white-space:pre-wrap; word-break:break-word; } /* 定义当前元素的样式规则。 */
.workflow-admin-panel { margin-bottom:18px; padding:12px; background:#fafcff; border:1px solid #d6e4ff; border-radius:6px; }.workflow-admin-toolbar { display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:10px; }.workflow-admin-toolbar>div:first-child { min-width:0; display:grid; gap:3px; }.workflow-admin-toolbar strong { color:#1f2329; font-size:13px; }.workflow-admin-toolbar small { color:#86909c; font-size:12px; }.workflow-admin-toolbar>div:last-child { display:flex; gap:6px; flex:none; }.workflow-admin-list { display:grid; gap:7px; }.workflow-admin-item { padding:10px; display:flex; align-items:flex-start; justify-content:space-between; gap:12px; background:#fff; border:1px solid #edf0f5; border-radius:5px; }.workflow-admin-main { min-width:0; display:grid; gap:4px; }.workflow-admin-main>div { min-width:0; display:flex; align-items:center; flex-wrap:wrap; gap:5px; }.workflow-admin-main strong { max-width:260px; overflow:hidden; color:#303133; font-size:12px; text-overflow:ellipsis; white-space:nowrap; }.workflow-admin-main small { overflow:hidden; color:#86909c; font-size:12px; text-overflow:ellipsis; white-space:nowrap; }.workflow-admin-main p { margin:2px 0 0; overflow:hidden; color:#646c73; font-size:12px; line-height:1.5; text-overflow:ellipsis; white-space:nowrap; }.workflow-admin-actions { display:flex; flex:none; align-items:center; gap:5px; }.workflow-admin-actions .el-button { margin-left:0; } /* 定义当前元素的样式规则。 */
.manifest-guide { margin:-2px 0 16px; overflow:hidden; background:#fbfcfe; border:1px solid #d6e4ff; border-radius:6px; }.manifest-guide-title { padding:12px 14px; display:flex; align-items:center; justify-content:space-between; gap:12px; background:#f0f5ff; border-bottom:1px solid #d6e4ff; }.manifest-guide-title>div { display:grid; gap:3px; }.manifest-guide-title strong { color:#10239e; font-size:13px; }.manifest-guide-title small { color:#697386; font-size:12px; }.manifest-field-list { display:grid; }.manifest-field { padding:10px 14px; display:grid; grid-template-columns:124px 74px minmax(0,1fr); align-items:start; gap:10px; border-bottom:1px solid #edf0f5; }.manifest-field:last-child { border-bottom:0; }.manifest-field code,.tool-whitelist code { color:#0958d9; font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace; font-size:12px; font-weight:700; word-break:break-all; }.manifest-field>span { width:max-content; padding:2px 6px; color:#595959; background:#f0f0f0; border-radius:3px; font-size:12px; }.manifest-field p { margin:0; color:#4e5969; font-size:12px; line-height:1.55; }.tool-whitelist { padding:13px 14px; display:grid; gap:10px; background:#fff; border-top:1px solid #d6e4ff; }.tool-whitelist>strong { color:#303133; font-size:12px; }.tool-whitelist>div { display:grid; grid-template-columns:1fr 1fr; gap:7px; }.tool-whitelist span { min-width:0; padding:8px 9px; display:grid; gap:3px; background:#f7f9fc; border-radius:4px; }.tool-whitelist small { color:#697386; font-size:12px; } /* 定义当前元素的样式规则。 */
.ai-chat-card :deep(.el-card__body) { display:flex; flex-direction:column; }.chat-header>div:last-child { display:flex; align-items:center; }.quick-prompts { flex:none; display:flex; flex-wrap:wrap; gap:7px; padding-bottom:12px; border-bottom:1px solid #f0f0f0; }.quick-prompts button { padding:6px 9px; color:#1554ad; background:#f5f9ff; border:1px solid #d6e8ff; border-radius:3px; font-size:12px; cursor:pointer; }.quick-prompts button:hover:not(:disabled) { color:#fff; background:#1677ff; border-color:#1677ff; }.quick-prompts button:disabled { opacity:.5; cursor:not-allowed; }.chat-log { min-height:0; flex:1; padding:15px 3px 6px; overflow:auto; overscroll-behavior:contain; }.message-row { display:flex; align-items:flex-start; gap:9px; }.message-row.user { flex-direction:row-reverse; }.message-avatar { width:28px; height:28px; flex:0 0 28px; display:grid; place-items:center; color:#fff; background:#3f4960; border-radius:4px; font-size:12px; font-weight:700; }.message-row.user .message-avatar { background:#1677ff; }.message-content { width:100%; min-width:0; max-width:min(82%,760px); margin-bottom:14px; }.message-row.user .message-content { width:fit-content; max-width:min(82%,760px); display:flex; flex-direction:column; align-items:flex-end; }.chat-message { width:100%; max-width:none; box-sizing:border-box; margin:0; padding:10px 12px; color:#3d3d3d; background:#f5f5f5; border-radius:4px; font-size:13px; line-height:1.75; word-break:break-word; }.message-row.user .chat-message { width:fit-content; max-width:100%; }.chat-message.user { color:#fff; background:#1677ff; }.chat-message.is-failed { background:#fff7f6; border:1px solid #ffccc7; }.chat-message.is-canceled { color:#646c73; background:#fafafa; border:1px dashed #d9d9d9; }.chat-message p { margin:0; white-space:pre-wrap; }.typing { min-width:150px; display:flex; align-items:center; gap:5px; color:#8c8c8c; }.typing i { width:5px; height:5px; background:#8c8c8c; border-radius:50%; animation:pulse 1s infinite; }.typing i:nth-child(2){animation-delay:.16s}.typing i:nth-child(3){animation-delay:.32s}.typing span { margin-left:4px; font-size:12px; }.message-error { margin-top:9px; padding-top:9px; display:grid; gap:3px; border-top:1px solid #ffccc7; }.message-error strong { color:#cf1322; font-size:12px; }.message-error small { color:#8c8c8c; font-size:12px; word-break:break-all; }.message-error .el-button { width:max-content; height:auto; margin-top:3px; padding:0; }.message-meta { margin-top:5px; display:flex; align-items:center; gap:8px; color:#8c8c8c; font-size:12px; }.message-meta button { padding:0; color:#1677ff; background:none; border:0; font-size:12px; cursor:pointer; }.chat-compose { flex:none; display:flex; align-items:flex-end; gap:9px; padding-top:11px; border-top:1px solid #f0f0f0; }.chat-compose .el-button { min-width:72px; }.chat-notice { margin-top:8px; color:#8c8c8c; text-align:center; } /* 定义当前元素的样式规则。 */
.rule-draft-card { margin-top:10px; padding:10px; display:grid; gap:7px; background:#fffbe6; border:1px solid #ffe58f; border-radius:4px; }.rule-draft-card>div { display:flex; align-items:center; justify-content:space-between; gap:8px; }.rule-draft-card small { color:#646c73; }.rule-draft-card .el-button { width:max-content; } /* 定义当前元素的样式规则。 */
.ai-workbench { grid-template-columns:minmax(250px,286px) minmax(0,1fr); } /* 定义当前元素的样式规则。 */
.control-section-label { margin:2px 0 10px; display:flex; align-items:center; gap:7px; color:#303133; font-size:12px; font-weight:700; letter-spacing:.02em; }.control-section-label span { width:22px; height:18px; display:grid; place-items:center; color:#1677ff; background:#eaf3ff; border-radius:3px; font-size:12px; } /* 定义当前元素的样式规则。 */
.runtime-overview { display:grid; grid-template-columns:1fr 1fr; gap:8px; }.overview-item { min-width:0; padding:10px; display:grid; gap:3px; color:inherit; text-align:left; background:#fafafa; border:1px solid #ededed; border-radius:4px; cursor:pointer; transition:border-color .15s,background .15s; }.overview-item:hover { background:#f5f9ff; border-color:#91caff; }.overview-item span { color:#8c8c8c; font-size:12px; }.overview-item strong { overflow:hidden; color:#303133; font-size:12px; text-overflow:ellipsis; white-space:nowrap; }.overview-item small { overflow:hidden; color:#646c73; font-size:12px; text-overflow:ellipsis; white-space:nowrap; }.runtime-warning { margin-top:10px; }.manager-entry { width:100%; margin-top:12px; padding:11px 12px; display:flex; align-items:center; justify-content:space-between; color:inherit; text-align:left; background:#fff; border:1px solid #d9d9d9; border-radius:4px; cursor:pointer; }.manager-entry:hover { background:#f5f9ff; border-color:#1677ff; }.manager-entry>span { display:grid; gap:2px; }.manager-entry strong { font-size:12px; }.manager-entry small { color:#8c8c8c; font-size:12px; }.manager-entry b { color:#1677ff; font-size:12px; } /* 定义当前元素的样式规则。 */
.runtime-overview-single { grid-template-columns:1fr; }.overview-item-static { cursor:default; }.overview-item-static:hover { background:#fafafa; border-color:#ededed; } /* 定义当前元素的样式规则。 */
.overview-item:first-child { background:#f9f0ff; border-color:#efdbff; }.overview-item:first-child:hover { border-color:#b37feb; }.overview-item:first-child strong { color:#531dab; }.overview-item:last-child { background:#f6ffed; border-color:#d9f7be; }.overview-item:last-child:hover { border-color:#95de64; }.overview-item:last-child strong { color:#237804; } /* 定义当前元素的样式规则。 */
.overview-item-static { background:#fafafa; border-color:#ededed; }.overview-item-static:hover { background:#fafafa; border-color:#ededed; }.overview-item-static strong { color:#303133; } /* 定义当前元素的样式规则。 */
.manager-panel { animation:manager-in .16s ease-out; }.manager-intro { margin-bottom:20px; padding:16px; background:#f5f9ff; border:1px solid #d6e8ff; border-left:4px solid #1677ff; border-radius:6px; }.manager-intro>span { color:#1677ff; font-size:12px; font-weight:700; letter-spacing:.12em; }.manager-intro>div { margin-top:5px; display:flex; align-items:center; justify-content:space-between; gap:12px; }.manager-intro h3 { margin:0; color:#1f2329; font-size:16px; }.manager-intro p { margin:7px 0 0; color:#646c73; font-size:12px; line-height:1.6; }.drawer-form :deep(.el-select),.drawer-form :deep(.el-radio-group) { width:100%; }.drawer-form :deep(.el-radio-button) { flex:1; }.drawer-form :deep(.el-radio-button__inner) { width:100%; }.workflow-manager :deep(.el-drawer__header) { margin-bottom:0; padding-bottom:16px; border-bottom:1px solid #ededed; }.workflow-manager :deep(.el-drawer__body) { padding-top:16px; }.agent-editor-dialog :deep(.el-dialog__body) { max-height:calc(100vh - 210px); overflow-y:auto; overscroll-behavior:contain; }.agent-editor-dialog .drawer-form { margin-top:14px; } /* 定义当前元素的样式规则。 */
...
.manager-menu-copy small { color:var(--muted-foreground); } /* 定义当前元素的样式规则。 */
...
.chat-message.is-canceled { color:#475569; } /* 定义当前元素的样式规则。 */
.knowledge-summary span, /* 定义当前元素的样式规则。 */
.knowledge-summary small { color:#3f6212; } /* 定义当前元素的样式规则。 */
.typing i { background:var(--muted-foreground); } /* 定义当前元素的样式规则。 */
.message-error .el-button { min-height:24px; height:24px; padding:0 8px; } /* 定义当前元素的样式规则。 */
