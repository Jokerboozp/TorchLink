...
const welcomeMessage = () => ({ id:'welcome', role:'assistant', status:'succeeded', text:'你好，我是消防物联网智能运维助手。选择一个工作流后，可以直接查询设备、告警和趋势；所有工具执行都会显示在运行轨迹中。', tools:[] })
...
function startCreateAgent() {
  ...
  focusAgentEditor()
  ElMessage.info('已打开新建智能体，请填写配置清单结构化数据')
}
...
async function saveAgent() {
  ...
  catch { ElMessage.error('智能体结构化数据格式不正确'); return }
  if (editingAgentId.value && manifest?.id !== editingAgentId.value) {
    ElMessage.error('编辑时不能修改智能体的唯一标识；如需新插件请先新建')
    return
  }
  ...
  try {
    ...
    agentEditorVisible.value = false
    ElMessage.success(`${editing ? '智能体已更新' : '智能体已创建'}：${workflowName(saved)}`)
...
<template>
  ...
  <el-drawer v-model="managementVisible" title="智能体管理" size="min(760px, 94vw)" class="workflow-manager" append-to-body>
  ...
  <el-dialog v-model="agentEditorVisible" :title="editingAgentId ? '编辑智能体' : '新建智能体'" width="min(820px, 94vw)" class="agent-editor-dialog" append-to-body destroy-on-close>
    <el-alert title="结构化数据标准不支持注释。内置智能体仅可查看，动态智能体可在管理清单中编辑、启用或删除。" type="info" :closable="false" show-icon />
    <el-form class="drawer-form" label-position="top" :disabled="!isAdmin || creatingAgent">
      <el-form-item label="智能体配置清单"><el-input ref="agentEditorRef" v-model="agentJson" class="agent-json-editor" type="textarea" :rows="18" resize="vertical" spellcheck="false" /></el-form-item>
...
.ai-runtime { flex:none; min-height:74px; margin-bottom:16px; padding:15px 18px; display:flex; align-items:center; justify-content:space-between; gap:20px; background:#fff; border:1px solid #e8e8e8; border-left:3px solid #1677ff; border-radius:4px; }.ai-runtime>div:first-child { display:grid; gap:3px; }.section-kicker { color:#1677ff; font-size:12px; font-weight:700; letter-spacing:.14em; }.ai-runtime strong { font-size:16px; }.ai-runtime small { color:var(--muted); }.runtime-actions { display:flex; align-items:center; gap:12px; }.runtime-status { display:flex; align-items:center; gap:8px; color:#646c73; font-size:13px; white-space:nowrap; }.runtime-status i { width:7px; height:7px; background:#ff4d4f; border-radius:50%; }.runtime-status i.online { background:#52c41a; }
...
.controls-toggle { display:none; } @media (max-width:1120px) {.ai-runtime { gap:10px; flex-wrap:wrap; }.runtime-status { white-space:normal; flex-wrap:wrap; }.quick-prompts { flex-wrap:nowrap; overflow-x:auto; }.quick-prompts button { flex:none; } }
@media (max-width:760px) { .controls-toggle { display:inline-flex; }.ai-workbench { grid-template-columns:1fr; }.control-card { display:none; }.ai-workbench.is-controls-open .ai-chat-card { display:none; }.ai-workbench.is-controls-open .control-card { display:flex; } }
...
.message-meta button { min-height:24px; padding:3px 8px; color:#1d4ed8; background:#eff6ff; border:1px solid #bfdbfe; border-radius:.375rem; font-size:12px; cursor:pointer; }
...
.ai-runtime { border:0; border-left:3px solid var(--primary); border-radius:var(--radius); }
