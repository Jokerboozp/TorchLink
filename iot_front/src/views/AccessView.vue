...
const user=reactive(userAccessPayload()) /* 声明 user。 */
const role=reactive({id:'',name:'',description:'',permissions:[]}) /* 声明 role。 */
const tenantId=ref(session.tenant),deviceOptions=ref([]),devicesLoading=ref(false),devicesError=ref('') /* 声明 tenantId。 */
...
const deviceSelectionPending = computed(() => ['user','devices'].includes(dialog.value) && user.deviceScope === 'selected' && (devicesLoading.value || !!devicesError.value))
...
function editUser(value, section='user') {
 editing.value=!!value
 Object.assign(user,userAccessPayload(value))
 dialog.value=section
 void loadDevices()
}
function enableDeviceMenu() { user.permissions=[...new Set([...user.permissions,'menu:devices'])] }
...
async function save(){if(saving.value||deviceSelectionPending.value)return;saving.value=true;try{const isUser=dialog.value==='user'||dialog.value==='devices',value=isUser?userAccessPayload(user):role;const base=isUser?'/api/v1/access/users':'/api/v1/access/roles';await api(base+(editing.value?'/'+encodeURIComponent(isUser?user.username:role.id):''),{method:editing.value?'PUT':'POST',body:JSON.stringify(value)});dialog.value='';user.password='';await load();UiMessage.success('已保存')}catch(e){notifyError(e)}finally{saving.value=false}} /* 定义 save 函数。 */
...
function userActions(row) {
  ...
  return [
    { key:'devices', label:'设备授权', permission:'PUT /api/v1/access/users/:id', disabled:self, onClick:() => editUser(row, 'devices') },
...
<template>
 ...
 <FilterBar>
  <p class="access-tenant">当前租户：{{tenantId}}。此处创建的用户归属该租户，登录时填写相同租户；内置管理员用于系统维护，不在此列表中。</p>
  <template #actions>
 ...
 <ui-dialog :model-value="dialog==='user'||dialog==='role'" :title="`${editing?'编辑':'添加'}${dialog==='user'?'用户':'角色'}`" width="min(850px,94vw)" :close-on-click-modal="false" @close="dialog='';user.password=''"> <!-- 渲染 ui-dialog 界面元素。 -->
  <ui-form v-if="dialog==='user'" class="user-editor" label-position="top" :disabled="saving">
   ...
   <section class="user-editor-section">
    ...
    <div class="user-editor-heading"><h3>可查看的设备</h3><p>按设备逐台授权，保存后同时限制设备、告警及助手查询的数据范围。</p></div>
    <DeviceScopePicker v-model:scope="user.deviceScope" v-model:device-ids="user.deviceIds" :devices="deviceOptions" :loading="devicesLoading" :error="devicesError" :disabled="saving" :can-view-devices="canViewDevices" @retry="loadDevices" @enable-device-menu="enableDeviceMenu" />
   </section>
 ...
 <ui-dialog :model-value="dialog==='devices'" :title="`设备授权 · ${user.displayName || user.username}`" width="min(720px,94vw)" :close-on-click-modal="false" @close="dialog=''">
  <p class="device-authorization-account">用户：{{ user.username }} · 租户：{{ tenantId }}</p>
  <DeviceScopePicker v-if="dialog==='devices'" v-model:scope="user.deviceScope" v-model:device-ids="user.deviceIds" :devices="deviceOptions" :loading="devicesLoading" :error="devicesError" :disabled="saving" :can-view-devices="canViewDevices" @retry="loadDevices" @enable-device-menu="enableDeviceMenu" />
  <template #footer><ui-button :disabled="saving" @click="dialog=''">取消</ui-button><ui-button type="primary" :loading="saving" :disabled="deviceSelectionPending" @click="save">保存设备授权</ui-button></template>
 </ui-dialog>
...
<style scoped>
.device-authorization-account { margin: 0 0 16px; color: var(--text-muted); font-size: 13px; }
