...
async function load(){loading.value=true;try{const [u,r,p,d]=await Promise.all([api('/api/v1/access/users'),api('/api/v1/access/roles'),api('/api/v1/access/permissions'),api('/api/v1/access/device-options')]);tenantId.value=u.tenantId;deviceOptions.value=d.items||[];users.value=u.items||[];roles.value=r.items||[];catalog.value=p.items||[]}catch(e){notifyError(e)}finally{loading.value=false}} /* 定义 load 函数。 */
 ...
 <ui-dialog :model-value="dialog==='user'||dialog==='role'" :title="`${editing?'编辑':'添加'}${dialog==='user'?'用户':'角色'}`" width="min(850px,94vw)" :close-on-click-modal="false" @close="dialog='';user.password=''"> <!-- 渲染 ui-dialog 界面元素。 -->
  <ui-form v-if="dialog==='user'" class="user-editor" label-position="top" :disabled="saving">
   ...
   <section class="user-editor-section">
    ...
    <ui-form-item label="设备访问范围"><ui-radio-group v-model="user.deviceScope" class="user-editor-scope"><ui-radio-button value="none">无设备</ui-radio-button><ui-radio-button value="selected">指定设备</ui-radio-button><ui-radio-button value="all">当前租户全部设备</ui-radio-button></ui-radio-group></ui-form-item>
  ...
  <ui-form v-else label-position="top" :disabled="saving"><ui-form-item label="角色标识" required><ui-input v-model="role.id" :disabled="editing" placeholder="例如 operations"/></ui-form-item><ui-form-item label="角色名称" required><ui-input v-model="role.name" placeholder="例如 设备运维"/></ui-form-item><ui-form-item label="说明"><ui-input v-model="role.description"/></ui-form-item><ui-form-item label="菜单与操作权限"><PermissionPicker v-model="role.permissions" :catalog="catalog"/></ui-form-item></ui-form> <!-- 渲染 ui-form 界面元素。 -->
