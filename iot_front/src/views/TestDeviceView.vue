...
async function prepare(reset = false) { /* 定义 prepare 函数。 */
  ...
  try { /* 执行当前语句并推进处理流程。 */
    ...
    if (reset) { /* 判断条件并选择处理分支。 */
      ...
      ElMessage.success('测试设备和默认报文已准备完成') /* 执行当前语句并推进处理流程。 */
    } /* 结束当前表达式或代码块。 */
...
async function sendTemplate(kind) { /* 定义 sendTemplate 函数。 */
  if (!device.value) return ElMessage.warning('测试设备还没有准备好') /* 判断条件并选择处理分支。 */
  let body /* 声明 body。 */
  try { /* 执行当前语句并推进处理流程。 */
  ...
  try { /* 执行当前语句并推进处理流程。 */
    ...
    ElMessage.success(`${templateNames[kind]}已发送，已进入平台处理链路`) /* 执行当前语句并推进处理流程。 */
  } catch (error) { /* 结束当前表达式或代码块。 */
...
<template>
  <div class="test-device-view"> <!-- 渲染 div 界面元素。 -->
    <div class="page-toolbar"> <!-- 渲染 div 界面元素。 -->
      <el-button v-permission="'POST /api/v1/test-devices/provision'" type="primary" :loading="loading" @click="prepare(false)">准备测试设备</el-button> <!-- 渲染 el-button 界面元素。 -->
      <el-button plain type="warning" :loading="loading" @click="resetLocalTemplates">恢复默认配置</el-button> <!-- 渲染 el-button 界面元素。 -->
      <el-button v-permission="'menu:devices'" @click="emit('navigate', 'devices')">查看设备管理</el-button> <!-- 渲染 el-button 界面元素。 -->
      <el-button v-permission="'menu:alarms'" @click="emit('navigate', 'alarms')">打开告警中心</el-button> <!-- 渲染 el-button 界面元素。 -->
    ...
    <div v-if="device" class="test-device-layout top-gap"> <!-- 渲染 div 界面元素。 -->
      <section class="test-device-workbench"> <!-- 渲染 section 界面元素。 -->
        <el-card shadow="never" class="surface-card"> <!-- 渲染 el-card 界面元素。 -->
          <template #header>
            <div class="card-header"> <!-- 渲染 div 界面元素。 -->
              <div><strong>测试设备</strong><small>已绑定产品、已发布协议包，可直接开始发送</small></div> <!-- 渲染 div 界面元素。 -->
              <el-tag type="success" round>设备链路已就绪</el-tag> <!-- 渲染 el-tag 界面元素。 -->
            </div> <!-- 结束当前界面区域。 -->
          </template>
        ...
        <el-card shadow="never" class="surface-card template-card">
          <template #header>
            <div class="card-header"><div><strong>报文模板</strong><small>{{ templateDescriptions[activeTemplate] }}</small></div><el-tag effect="plain" round>{{ currentTemplateName }}</el-tag></div> <!-- 渲染 div 界面元素。 -->
          </template>
      ...
      <aside class="test-device-side"> <!-- 渲染 aside 界面元素。 -->
        <el-card shadow="never" class="surface-card"> <!-- 渲染 el-card 界面元素。 -->
          ...
          <el-steps direction="vertical" :active="4"> <!-- 渲染 el-steps 界面元素。 -->
            <el-step title="发送正常数据" description="确认原始报文归档、标准消息和设备在线状态。" /> <!-- 渲染 el-step 界面元素。 -->
            <el-step title="发送报警数据" description="确认设备告警直接进入告警中心，并验证规则联动（如已配置）。" /> <!-- 渲染 el-step 界面元素。 -->
            <el-step title="发送恢复数据" description="确认告警从活动中变为已恢复。" /> <!-- 渲染 el-step 界面元素。 -->
            <el-step title="修改模板再发送" description="验证自定义字段、事件和异常报文。" /> <!-- 渲染 el-step 界面元素。 -->
          </el-steps> <!-- 结束当前界面区域。 -->
        </el-card> <!-- 结束当前界面区域。 -->

        <el-card shadow="never" class="surface-card"> <!-- 渲染 el-card 界面元素。 -->
          <template #header><div class="card-header"><strong>最近发送</strong><small>本次打开页面的记录</small></div></template>
          <el-empty v-if="!history.length" description="还没有发送记录" :image-size="58" /> <!-- 渲染 el-empty 界面元素。 -->
          <div v-for="item in history" :key="item.id" class="send-history-item"> <!-- 渲染 div 界面元素。 -->
            ...
            <el-tag :type="item.alarm ? 'danger' : (item.parsed ? 'success' : 'warning')" round>{{ item.alarm ? '已触发告警' : (item.parsed ? messageTypeLabel(item.messageType) : '处理中') }}</el-tag> <!-- 渲染 el-tag 界面元素。 -->
          </div> <!-- 结束当前界面区域。 -->
        </el-card> <!-- 结束当前界面区域。 -->

        <el-card v-if="result" shadow="never" class="surface-card result-card"> <!-- 渲染 el-card 界面元素。 -->
          ...
          <el-alert v-if="result.error" title="发送失败" :description="result.error" type="error" :closable="false" show-icon /> <!-- 渲染 el-alert 界面元素。 -->
          <template v-else>
            <el-descriptions :column="1" border> <!-- 渲染 el-descriptions 界面元素。 -->
              <el-descriptions-item label="消息编号"><code>{{ result.messageId }}</code></el-descriptions-item> <!-- 渲染 el-descriptions-item 界面元素。 -->
              <el-descriptions-item label="解析状态">{{ result.rawDetail?.parseStatus ? statusLabel(result.rawDetail.parseStatus) : '已提交' }}</el-descriptions-item> <!-- 渲染 el-descriptions-item 界面元素。 -->
              <el-descriptions-item label="标准消息">{{ result.rawDetail?.standardMessage ? `${messageTypeLabel(result.rawDetail.standardMessage.messageType)}（${result.rawDetail.standardMessage.messageType}）` : '等待处理' }}</el-descriptions-item> <!-- 渲染 el-descriptions-item 界面元素。 -->
              <el-descriptions-item label="关联告警">{{ result.alarms?.length ? `${result.alarms.length} 条 · ${alarmLabel(result.alarms[0].alarmType)}` : '暂无' }}</el-descriptions-item> <!-- 渲染 el-descriptions-item 界面元素。 -->
