...
async function send(repeat = false) {
  ...
  try {
    ...
    if (!repeat) {
      ...
      if (!data || Array.isArray(data) || typeof data !== 'object' || !Object.keys(data).length) throw new Error('data 必须是非空结构化数据对象')
...
function exportReport() {
  const report = { exportedAt: new Date().toISOString(), deviceId: form.deviceId, records: records.value, scope: '浏览器标准设备联调；消息服务确认与平台解析分别记录，不代表厂商真机验收。' }
...
<template>
  <el-card class="standard-commissioning" shadow="never">
    ...
    <p>事件：{"type":"heartbeat"}；状态：{"connectionStatus":"CONNECTED"}；命令回执：{"commandId":"收到的命令标识","success":true}。</p>
