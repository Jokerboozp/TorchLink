...
async function inspect(row){try{runtime.value=await api(`/api/v1/edge-nodes/${encodeURIComponent(row.id)}/runtime`);targetNode.value=row.id;profiles.value=(await api('/api/v2/device-access-profiles')).items.filter(p=>p.mode!=='listener')}catch(e){notifyError(e)}}
...
<p>登记节点后生成一次性凭据，在现场启动 Agent。当前 Agent 支持 Modbus TCP 持续采集和 HTTPS 持久化补传；配置分配不代表已经连接。</p>
