...
<div v-if="runtime"><p>运行状态：{{({ONLINE:'在线',OFFLINE:'离线',WAITING:'等待心跳'})[runtime.status]}} · 最后心跳：{{formatTime(runtime.heartbeat?.lastSeenAt)}} · 待补传：{{runtime.heartbeat?.queueDepth||0}} · 待处置拒收：{{runtime.heartbeat?.rejectedDepth||0}} · 损坏隔离：{{runtime.heartbeat?.corruptDepth||0}}</p><p>{{runtime.heartbeat?.lastError}}</p><p v-if="runtime.heartbeat?.corruptDepth" class="text-warning">损坏文件已保留在现场节点的 outbox 目录，其他数据继续补传。请备份并检查磁盘与原始文件；重试拒收不会重新发送损坏文件。</p><el-select v-model="selectedProfile" placeholder="选择已有采集或监听配置"><el-option v-for="p in profiles" :key="p.id" :value="p.id" :label="`${p.deviceId} · ${p.host}:${p.port}`"/></el-select><el-button :disabled="!selectedProfile" :loading="busy" @click="assign">分配到此节点</el-button><el-button @click="inspect({id:targetNode})">刷新心跳</el-button></div>
