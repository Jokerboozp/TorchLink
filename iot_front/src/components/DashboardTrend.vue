...
<template>
  ...
  <div v-if="total" class="trend-plot"> <!-- 渲染 div 界面元素。 -->
    <svg viewBox="0 0 700 245" role="group" aria-label="每日新增告警趋势"> <!-- 渲染 svg 界面元素。 -->
      <defs><linearGradient id="dashboard-trend-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#0071e3" stop-opacity=".18" /><stop offset="100%" stop-color="#0071e3" stop-opacity=".01" /></linearGradient></defs> <!-- 渲染 defs 界面元素。 -->
      ...
      <polyline :points="chart.line" fill="none" stroke="#0071e3" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round" /> <!-- 渲染 polyline 界面元素。 -->
      <g v-for="(point,index) in chart.points" :key="point.date"> <!-- 渲染 g 界面元素。 -->
        <circle :cx="point.x" :cy="point.y" :r="selected === point.date ? 5 : 3" fill="white" stroke="#0071e3" stroke-width="2" /> <!-- 渲染 circle 界面元素。 -->
