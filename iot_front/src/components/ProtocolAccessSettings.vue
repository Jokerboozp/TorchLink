...
<section class="protocol-access-settings"> <!-- 渲染 section 界面元素。 -->
 ...
 <div v-for="(query,index) in profile.queries||[]" :key="index" class="setting-row"> <!-- 渲染 div 界面元素。 -->
  <ui-input v-model="query.type" placeholder="查询类型，例如 read-status" aria-label="查询类型" /> <!-- 渲染 ui-input 界面元素。 -->
 ...
 <ui-button :disabled="(profile.queries?.length||0)>=32" @click="addQuery">添加定时查询</ui-button></template> <!-- 渲染 ui-button 界面元素。 -->
