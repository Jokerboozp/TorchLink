...
function rename(key, next) { /* 定义 rename 函数。 */
  if (!next || entries.value.some(([k]) => k !== key && k === next)) return UiMessage.warning('参数名称不能为空或重复') /* 判断条件并选择处理分支。 */
...
<template>
  <ui-input v-if="kind === 'string'" :model-value="modelValue" :aria-label="label" @update:model-value="emit('update:modelValue',$event)" /> <!-- 渲染 ui-input 界面元素。 -->
  <ui-input-number v-else-if="kind === 'number' || kind === 'integer'" :model-value="modelValue" :precision="kind === 'integer' ? 0 : undefined" :aria-label="label" @update:model-value="emit('update:modelValue',$event)" /> <!-- 渲染 ui-input-number 界面元素。 -->
  <ui-select v-else-if="kind === 'boolean'" :model-value="modelValue" :aria-label="label" clearable placeholder="请选择" @update:model-value="emit('update:modelValue', $event === '' ? null : $event)"><ui-option :value="true" label="是（true）"/><ui-option :value="false" label="否（false）"/></ui-select> <!-- 渲染 ui-select 界面元素。 -->
  ...
  <div v-else-if="kind === 'object' || kind === 'array'" class="command-object"> <!-- 渲染 div 界面元素。 -->
    ...
    <ui-button size="small" @click="add">{{kind === 'array' ? '添加列表项' : '添加参数项'}}</ui-button> <!-- 渲染 ui-button 界面元素。 -->
    <ui-button v-if="modelValue != null" size="small" @click="emit('update:modelValue',null)">清除</ui-button> <!-- 渲染 ui-button 界面元素。 -->
    <ui-button v-else size="small" @click="emit('update:modelValue',empty(kind))">设为空{{kind === 'array' ? '列表' : '对象'}}</ui-button> <!-- 渲染 ui-button 界面元素。 -->
  </div> <!-- 结束当前界面区域。 -->
</template>
<style scoped>
