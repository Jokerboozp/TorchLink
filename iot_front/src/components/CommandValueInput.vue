...
function rename(key, next) {
  if (!next || entries.value.some(([k]) => k !== key && k === next)) return ElMessage.warning('参数名称不能为空或重复')
...
<template>
  <el-input v-if="kind === 'string'" :model-value="modelValue" :aria-label="label" @update:model-value="emit('update:modelValue',$event)" />
  <el-input-number v-else-if="kind === 'number' || kind === 'integer'" :model-value="modelValue" :precision="kind === 'integer' ? 0 : undefined" :aria-label="label" @update:model-value="emit('update:modelValue',$event)" />
  <el-select v-else-if="kind === 'boolean'" :model-value="modelValue" :aria-label="label" clearable placeholder="请选择" @update:model-value="emit('update:modelValue', $event === '' ? null : $event)"><el-option :value="true" label="是（true）"/><el-option :value="false" label="否（false）"/></el-select>
