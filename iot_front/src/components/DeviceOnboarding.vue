...
const availableProfiles = computed(() => profiles.value.filter(x=>x.profile && !x.profile.edgeNodeId && x.type===form.type && x.profile.productId===form.productId && x.profile.enabled))
const form = reactive({ productId:'', deviceId:'', name:'', type:'MQTT', protocolId:'', protocolVersion:'', messageKind:'property',pollIntervalSec:10, profile:{host:'0.0.0.0',port:26875,unitId:1,timeoutMs:3000,retries:0,autoRegister:false,collectorId:''} })
...
const modbus = computed(() => form.type === 'MODBUS_TCP')
