...
async function test(){busy.value=true;preview.value=null;try{preview.value=await api('/api/v1/onboarding/test',{method:'POST',body:JSON.stringify(request())})}catch(e){if(e.testResult)preview.value=e.testResult;else notifyError(e)}finally{busy.value=false}}
