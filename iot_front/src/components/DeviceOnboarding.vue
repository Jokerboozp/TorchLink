...
<template>
  ...
  <el-dialog :model-value="true" title="添加设备" width="min(960px, 96vw)" :close-on-click-modal="false" :close-on-press-escape="!busy" :show-close="!busy" @close="emit('close')">
    ...
    <div class="onboarding-body" v-loading="busy">
      ...
      <div v-if="step===1"><el-radio-group v-model="form.type" class="connector-options"><el-radio v-for="kind in kinds" :key="kind.value" :value="kind.value" border>{{kind.label}}</el-radio></el-radio-group><p>现场采集或监听可在参数配置的高级选项中分配边缘节点。</p><p><el-button link @click="emit('navigate','cameras')">视频设备：打开摄像头管理</el-button></p></div>
      <el-form v-if="step===2" label-position="top">
        ...
        <div v-if="advanced&&!standard"><el-form-item label="协议（默认使用产品现有绑定）"><el-select v-model="selected" clearable filterable><el-option v-for="r in published" :key="r.protocolId+'@'+r.version" :value="r.protocolId+'@'+r.version" :label="`${r.name} · ${r.version}`" /></el-select></el-form-item><el-form-item label="超时（毫秒）"><el-input-number v-model="form.profile.timeoutMs" :min="1" :max="10000" /></el-form-item><el-form-item label="现场边缘节点"><el-select v-model="form.profile.edgeNodeId" :clearable="!serial" :disabled="!activeRead&&!listener"><el-option v-for="edge in edgeNodes.filter(e=>e.status==='ENABLED')" :key="edge.id" :value="edge.id" :label="edge.name"/></el-select><el-button link @click="edgeDialog=true">管理节点</el-button></el-form-item><el-form-item label="采集器"><el-input v-model="form.profile.collectorId" /></el-form-item><el-form-item v-if="listener" label="自动注册后续设备"><el-switch v-model="form.profile.autoRegister" /></el-form-item></div>
      </el-form>
      ...
      <div v-if="step===4&&preview"><el-alert :closable="false" type="info" :title="preview.message"/><el-descriptions :column="2" border><el-descriptions-item label="协议标识">{{preview.protocolId}}</el-descriptions-item><el-descriptions-item label="协议版本">{{preview.protocolVersion}}</el-descriptions-item><el-descriptions-item label="数据来源">{{['network-read','edge-read'].includes(preview.source)?'目标设备/模拟器读取':'样例预览'}}</el-descriptions-item><el-descriptions-item label="解析器">{{preview.parser}}</el-descriptions-item></el-descriptions><h4>原始报文</h4><pre>{{pretty(preview.raw)}}</pre><template v-if="preview.rawRequest"><h4>请求报文 / 响应报文</h4><pre>{{preview.rawRequest}}
