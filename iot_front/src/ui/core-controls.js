...
export const UiInput = defineComponent({ /* 文本、密码与多行输入共用 Naive UI 输入框。 */
  ...
  setup(props, { attrs, emit }) { return () => h(NInput, { ...attrs, class: ['el-input', attrs.class], value: props.modelValue == null ? '' : String(props.modelValue), type: props.type === 'textarea' ? 'textarea' : props.type === 'password' ? 'password' : 'text', rows: props.rows, autosize: props.autosize, placeholder: props.placeholder, disabled: props.disabled, readonly: props.readonly, clearable: props.clearable, maxlength: props.maxlength, showPasswordOn: props.showPassword ? 'mousedown' : undefined, 'onUpdate:value': value => { emit('update:modelValue', value); emit('input', value); if (value === '') emit('clear') }, onChange: value => emit('change', value) }) } /* 将 Naive UI 输入值交回业务表单。 */
}) /* 结束文本输入适配。 */
...
export const UiCard = defineComponent({ /* 管理卡片采用模板同款简洁边框。 */
  ...
  setup(props, { attrs, slots }) { return () => h(NCard, { ...attrs, class: ['el-card', attrs.class], title: props.header, bordered: true, size: 'medium' }, slots) } /* 将页头和正文插槽交给 Naive UI。 */
}) /* 结束卡片适配。 */
...
export const UiTag = defineComponent({ /* 状态标签统一使用 Naive UI。 */
  ...
  setup(props, { attrs, slots }) { return () => h(NTag, { ...attrs, class: ['el-tag', attrs.class], type: kind(props.type), round: props.round, bordered: props.effect === 'plain', size: props.size || 'small' }, slots) } /* 绘制状态标签。 */
}) /* 结束标签适配。 */
...
export const UiEmpty = defineComponent({ /* 空状态统一使用 Naive UI。 */
  ...
  setup(props, { attrs, slots }) { return () => h(NEmpty, { ...attrs, class: ['el-empty', attrs.class], description: props.description }, slots) } /* 绘制空状态。 */
}) /* 结束空状态适配。 */
...
export const UiForm = defineComponent({ /* 表单使用 Naive UI 的标签和校验结构。 */
  ...
  setup(props, { attrs, slots }) { return () => h(NForm, { ...attrs, class: ['el-form', props.inline && 'ui-form-inline', attrs.class], model: props.model, labelPlacement: props.labelPosition === 'top' ? 'top' : 'left', disabled: props.disabled, showFeedback: false }, slots) } /* 绘制表单。 */
}) /* 结束表单适配。 */
...
export const UiFormItem = defineComponent({ /* 表单字段保持原有标签文案。 */
  ...
  setup(props, { attrs, slots }) { return () => h(NFormItem, { ...attrs, class: ['el-form-item', attrs.class], label: props.label, path: props.prop, required: props.required, showFeedback: false }, slots) } /* 绘制字段布局。 */
}) /* 结束字段适配。 */
...
export const UiDescriptions = defineComponent({ /* 详情键值列表直接使用 Naive UI。 */
  ...
  setup(props, { attrs, slots }) { return () => { /* 把业务包装字段展开成 Naive UI 直接子项。 */
    ...
    return h(NDescriptions, { ...attrs, class: ['el-descriptions', attrs.class], column: props.column || 1, bordered: props.border, size: props.size || 'small', labelPlacement: 'left' }, { default: () => items }) /* 绘制详情列表。 */
  } } /* 结束详情渲染。 */
}) /* 结束详情适配。 */
...
export const UiCheckbox = defineComponent({ /* 独立勾选框保留布尔双向绑定。 */
  ...
  setup(props, { attrs, slots, emit }) { return () => h(NCheckbox, { ...attrs, class: ['el-checkbox', attrs.class], checked: props.modelValue, disabled: props.disabled, 'onUpdate:checked': value => { emit('update:modelValue', value); emit('change', value) } }, { default: () => slots.default?.() || props.label }) } /* 绘制勾选框。 */
}) /* 结束勾选框适配。 */
...
export const UiProgress = defineComponent({ /* 进度条使用 Naive UI。 */
  ...
  setup(props, { attrs }) { return () => h(NProgress, { ...attrs, class: ['el-progress', attrs.class], type: 'line', percentage: props.percentage || 0, status: props.status === 'exception' ? 'error' : props.status || 'default', height: props.strokeWidth || 10, showIndicator: props.showText }) } /* 绘制进度条。 */
}) /* 结束进度条适配。 */
