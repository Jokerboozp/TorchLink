...
模型管理默认预填 DeepSeek 地址与模型，管理员只需填写 API Key、测试并应用。未填写时显示待配置，不阻断设备业务；内置 Ollama 只提供知识库嵌入。配置的持久化、升级和联网边界见 [模型管理](../docs/DEPLOYMENT.md#模型管理与工作流服务)。原始报文支持设备、接入方式、解析状态、时间及报文标识等组合条件，权限仍由后端设备范围校验。
