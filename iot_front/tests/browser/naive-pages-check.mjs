...
try { /* 所有浏览器资源在 finally 中释放。 */
  ...
  for (const name of pages) { /* 逐页检查标题、正文和脚本异常。 */
    ...
    if (name==='智能助手') assert.ok(await evaluate("(() => {const workbench=document.querySelector('.ai-workbench'),chat=document.querySelector('.ai-chat-card'),select=document.querySelector('.chat-workflow-select'),help=document.querySelector('.chat-workflow-control small');return workbench&&chat&&select&&help&&!document.querySelector('.control-card')&&chat.getBoundingClientRect().width>=workbench.getBoundingClientRect().width-2&&select.getBoundingClientRect().width>=220&&help.getBoundingClientRect().top>=select.getBoundingClientRect().bottom&&!document.querySelector('.chat-header').innerText.includes('conversation_')})()"), '智能助手工作流选择与说明挤在一起，或仍显示技术会话编号')
