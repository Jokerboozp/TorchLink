...
try { /* 所有浏览器资源在 finally 中释放。 */
  ...
  for (const name of pages) { /* 逐页检查标题、正文和脚本异常。 */
    ...
    if (name==='运行总览') { assert.ok(await evaluate("(() => {const cards=[...document.querySelectorAll('.stat-card')];return cards.length===4 && new Set(cards.map(card=>getComputedStyle(card).borderTopColor)).size===1 && getComputedStyle(cards[0]).borderTopColor===getComputedStyle(cards[0]).borderLeftColor && getComputedStyle(document.querySelector('.device-ring circle')).transitionProperty.includes('stroke-dasharray')})()"), '仪表盘统计卡片应为中性样式，圆环过渡需生效'); assert.ok(!contrastAudit.some(item=>item.text==='已登记设备'&&item.contrast<4.5), '总览统计卡片的说明文字对比不足') } /* 检查总览主题与说明可读性。 */
