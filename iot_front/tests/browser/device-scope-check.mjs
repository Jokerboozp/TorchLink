...
try{
 ...
 await evaluate(`document.querySelector('.nav-item[aria-label="用户与权限"]').click()`);await delay(600)
 await click('添加用户');await fill('用户名',username);await fill('显示名称','演示 · 指定设备用户');await fill('初始密码',secret)
 if(!await evaluate(`document.querySelector('.ui-dialog input[disabled]')?.value===${JSON.stringify(tenant)}`))throw Error('新增用户未显示租户')
 await evaluate(`document.querySelector('input[value="selected"]').click()`)
 await evaluate(`(()=>{const row=[...document.querySelectorAll('.ui-form-item')].find(e=>e.querySelector('label')?.textContent==='可访问设备');row.querySelector('.n-base-selection').click()})()`)
 await until(()=>evaluate(`(()=>{const e=[...document.querySelectorAll('.n-base-select-option')].find(e=>e.textContent.trim().endsWith(${JSON.stringify('（'+allowed.id+'）')})&&e.getClientRects().length);e?.click();return !!e})()`))
 ...
 await call('Page.reload');await until(()=>evaluate(`!!document.querySelector('.nav-item[aria-label="设备管理"]')`))
 await evaluate(`document.querySelector('.nav-item[aria-label="设备管理"]').click()`);await delay(700)
 ...
 await evaluate(`document.querySelector('.nav-item[aria-label="告警中心"]').click()`);await delay(700)
