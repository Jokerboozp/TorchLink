...
try {
  ...
  assert.equal(await evaluate(`document.querySelector('.ui-drawer').textContent.includes('设备影子') || document.querySelector('.ui-drawer').textContent.includes('设备孪生与拓扑')`),false,'removed features must not have controls')
  assert.ok(await evaluate(`document.querySelector('.device-access-info .ui-descriptions')?.textContent.includes('/api/v1/device-ingest/')`),'access information must be in structured cells')
  assert.ok(await evaluate(`document.querySelector('.device-properties .ui-table')?.textContent.includes('42')`),'properties must be in a table')
  await evaluate('window.__failHistory=true')
  await until(()=>evaluate(`(()=>{const e=[...document.querySelectorAll('.device-connection-drawer button')].find(e=>e.textContent.trim()==='刷新'&&!e.disabled);if(!e)return false;e.click();return true})()`))
  ...
  await delay(300)
  assert.ok(await evaluate(`(()=>{const e=document.querySelector('.n-drawer-body-content-wrapper');return e.scrollWidth<=e.clientWidth+1})()`),'drawer content must not overflow')
  assert.ok(await evaluate(`[...document.querySelectorAll('.device-summary .n-descriptions-table tr')].every(e=>e.children.length===2)`),'mobile overview must use one label/value pair per row')
  assert.ok(await evaluate(`(()=>{const card=document.querySelector('.connection-section');const body=getComputedStyle(document.querySelector('.device-connection-drawer .n-drawer-body-content-wrapper')).backgroundColor;return getComputedStyle(card).backgroundColor!==body && getComputedStyle(card).borderTopWidth!=='0px'})()`),'card boundaries must be distinct from background')
  ...
  await call('Page.navigate',{url:process.env.IOT_TEST_ORIGIN})
  await click('设备管理');await click('详情')
  await until(()=>evaluate(`document.querySelector('.ui-drawer .ui-descriptions')`))
  assert.equal(await evaluate(`document.querySelector('.ui-drawer').textContent.includes('重新生成凭据')`),false,'viewer must not be offered credential mutation')
