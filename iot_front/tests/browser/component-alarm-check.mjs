...
try {
  ...
  assert.equal(await evaluate(`document.querySelectorAll('.el-table__body tbody tr').length`),2)
  await click('查看详情')
  await until(()=>evaluate(`document.querySelector('.el-dialog__body')?.textContent.includes('loop-1/node-7')`))
  assert.equal(await evaluate(`document.querySelector('.el-dialog__body').textContent.includes('部件位置')`),true)
  assert.equal(await evaluate(`document.querySelector('.el-dialog__body').textContent.includes('loop-1/node-8')`),false)
  await call('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true})
  await delay(200)
  assert.equal(await evaluate(`document.documentElement.scrollWidth<=window.innerWidth+2`),true)
  await click('关闭详情')
  await evaluate(`(async()=>{const result=await (await fetch('/api/v1/alarms?status=ACTIVE',{headers:{Authorization:'Bearer '+localStorage.getItem('iot_token')}})).json();for(const item of result.items)window.dispatchEvent(new CustomEvent('iot:realtime',{detail:{topic:'/iot/alarm/raised/c/d/b/smoke/controller',payload:item}}))})()`)
  ...
  console.log('PASS: authenticated component alarm list/detail, correct part and location, no normal-part alarm, same-message fire/fault popups remain separate, narrow viewport')
} finally {
