...
try { /* 执行当前语句并推进处理流程。 */
  ...
  assert.equal(await evaluate(`document.querySelectorAll('.el-table__body tbody tr').length`),2) /* 验证实际结果符合预期。 */
  await click('查看详情') /* 等待异步操作完成。 */
  await until(()=>evaluate(`document.querySelector('.el-dialog__body')?.textContent.includes('loop-1/node-7')`)) /* 等待异步操作完成。 */
  assert.equal(await evaluate(`document.querySelector('.el-dialog__body').textContent.includes('部件位置')`),true) /* 验证实际结果符合预期。 */
  assert.equal(await evaluate(`document.querySelector('.el-dialog__body').textContent.includes('loop-1/node-8')`),false) /* 验证实际结果符合预期。 */
