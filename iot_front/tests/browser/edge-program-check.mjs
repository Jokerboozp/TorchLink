...
try {
  ...
  await click('设置目标版本')
  await until(()=>evaluate(`document.querySelector('.el-message-box')?.textContent.includes('采集短暂停止')`))
  await click('确认设置')
  await until(async()=>{await click('刷新状态');return evaluate(`document.body.textContent.includes('请求版本号：4') && document.body.textContent.includes('状态对应请求：4') && document.body.textContent.includes('运行中')`)})
  await call('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true})
  assert.equal(await evaluate(`!![...document.querySelectorAll('.el-dialog')].find(e=>e.textContent.includes('目标程序版本')&&e.getBoundingClientRect().width<=390)`),true)
  const state=await evaluate(`fetch('/api/v1/edge-nodes/node/program',{headers:{Authorization:'Bearer '+localStorage.getItem('iot_token')}}).then(r=>r.json())`)
  assert.equal(state.status.version,'v2')
  assert.equal(state.status.phase,'RUNNING')
  assert.equal(state.generation,4)
  console.log('PASS: real launcher rollback shown, explicit upgrade target confirmation, authenticated actual version, generation and narrow-screen dialog')
