...
try {
  ...
  await click('摄像头映射')
  await click('新增摄像头')
  await until(()=>evaluate(`(()=>{const e=[...document.querySelectorAll('.el-collapse-item__header')].find(e=>e.textContent.trim()==='从 ONVIF 摄像头读取基础信息');if(!e)return false;e.click();return true})()`))
  await until(()=>evaluate(`(()=>{const item=[...document.querySelectorAll('.el-form-item')].find(e=>e.querySelector('label')?.textContent.trim()==='现场节点');const e=item?.querySelector('.el-select__wrapper');if(!e)return false;e.click();return true})()`))
  await until(()=>evaluate(`(()=>{const e=[...document.querySelectorAll('.el-select-dropdown__item')].find(e=>e.textContent.trim()==='edge'&&e.getClientRects().length);if(!e)return false;e.click();return true})()`))
  await fill('发现网卡 IPv4（节点本地白名单）','127.0.0.1')
  await click('发现候选设备')
  await until(()=>evaluate(`document.querySelector('.el-dialog')?.textContent.includes('发现结果尚未认证')`))
  assert.equal(await evaluate(`(()=>{const item=[...document.querySelectorAll('.el-form-item')].find(e=>e.querySelector('label')?.textContent.trim()==='摄像头 ID');return item?.querySelector('input')?.value||''})()`),'')
  await click('选择地址')
  assert.equal(await evaluate(`(()=>{const item=[...document.querySelectorAll('.el-form-item')].find(e=>e.querySelector('label')?.textContent.trim()==='摄像头地址');return item?.querySelector('input')?.value})()`),'127.0.0.1')
  await fill('现场凭据引用','missing')
  await click('读取信息')
  await until(()=>evaluate(`document.body.textContent.includes('local connection credential reference was not found')`))
  assert.equal(await evaluate(`(()=>{const item=[...document.querySelectorAll('.el-form-item')].find(e=>e.querySelector('label')?.textContent.trim()==='摄像头 ID');return item?.querySelector('input')?.value||''})()`),'')
  await fill('现场凭据引用','camera')
  await click('读取信息')
  await until(()=>evaluate(`(()=>{const item=[...document.querySelectorAll('.el-form-item')].find(e=>e.querySelector('label')?.textContent.trim()==='摄像头 ID');return item?.querySelector('input')?.value==='CAM-42'})()`))
  ...
  await until(()=>evaluate(`document.querySelector('.el-table')?.textContent.includes('CAM-42')`))
  console.log('PASS: ONVIF actual UDP discovery, untrusted candidate selection, browser node selection, rejected local credential reference, authenticated TLS/Digest read, metadata fill, narrow dialog and explicit save')
} finally {
