...
try {
  ...
  await click('摄像头映射')
  await click('新增摄像头')
  await until(()=>evaluate(`(()=>{const e=[...document.querySelectorAll('.el-collapse-item__header')].find(e=>e.textContent.trim()==='从网络视频标准摄像头读取基础信息');if(!e)return false;e.click();return true})()`))
  ...
  await fill('发现网卡网络地址（节点本地白名单）','127.0.0.1')
  await click('发现候选设备')
  await until(()=>evaluate(`document.querySelector('.el-dialog')?.textContent.includes('发现结果尚未认证')`))
  assert.equal(await evaluate(`(()=>{const item=[...document.querySelectorAll('.el-form-item')].find(e=>e.querySelector('label')?.textContent.trim()==='摄像头标识');return item?.querySelector('input')?.value||''})()`),'')
  await click('选择地址')
  ...
  await click('读取信息')
  await until(()=>evaluate(`document.body.textContent.includes('未找到现场连接凭据，请检查节点中的凭据名称')`))
  assert.equal(await evaluate(`(()=>{const item=[...document.querySelectorAll('.el-form-item')].find(e=>e.querySelector('label')?.textContent.trim()==='摄像头标识');return item?.querySelector('input')?.value||''})()`),'')
  await fill('现场凭据引用','camera')
  await click('读取信息')
  await until(()=>evaluate(`(()=>{const item=[...document.querySelectorAll('.el-form-item')].find(e=>e.querySelector('label')?.textContent.trim()==='摄像头标识');return item?.querySelector('input')?.value==='CAM-42'})()`))
