...
try{
 ...
 await setSession(auth.accessToken,'admin',user,['*']);await call('Page.reload');await until(()=>evaluate(`!!document.querySelector('.menu-item[aria-label="用户与权限"]')`));await delay(400)
 await evaluate(`document.querySelector('.collapse-button').click()`);await delay(250)
 if(!await evaluate(`document.querySelector('.app-aside').getBoundingClientRect().width<80`))throw Error('侧栏未折叠')
 await call('Page.reload');await until(()=>evaluate(`!!document.querySelector('.collapse-button')`));await delay(250)
 if(!await evaluate(`document.querySelector('.app-aside').getBoundingClientRect().width<80`))throw Error('侧栏折叠状态未保留')
 ...
 if(await evaluate(`[...document.querySelectorAll('.el-dropdown-menu')].filter(e=>e.getClientRects().length).some(e=>e.innerText.includes('租户'))`))throw Error('用户下拉仍显示租户')
 await evaluate(`document.querySelector('.account').click()`)
 await evaluate(`document.querySelector('.menu-item[aria-label="用户与权限"]').click()`);await delay(800)
 ...
 await until(()=>evaluate(`(()=>{const item=[...document.querySelectorAll('.el-select-dropdown__item')].find(e=>e.textContent.trim()===${JSON.stringify('演示 · 设备查看角色 '+suffix)}&&e.getClientRects().length);item?.click();return !!item})()`))
 await click('保存');await until(()=>evaluate(`![...document.querySelectorAll('.el-dialog')].some(e=>e.getClientRects().length)`))
