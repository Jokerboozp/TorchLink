...
try {
  ...
  const request = async(path,body) => {
    const response = await fetch(process.env.IOT_TEST_ORIGIN+path,{method:'POST',headers:{Authorization:`Bearer ${process.env.IOT_TEST_TOKEN}`,'Content-Type':'application/json'},body:JSON.stringify(body)})
  ...
  const preview=await request('/api/v1/onboarding/test',nativeRequest)
  await request('/api/v1/onboarding',{...nativeRequest,testToken:preview.testToken})
  await click('产品管理'); await click('新建产品')
  await fill('产品名称','浏览器标准产品'); await fill('产品标识','browser-standard-product')
  await click('保存产品')
  await until(()=>evaluate(`!document.querySelector('.el-dialog')?.getClientRects().length`))
  await click('设备管理'); await click('添加设备')
  assert.equal(await evaluate(`!!document.querySelector('.el-dialog .el-steps')`),false)
  await evaluate(`document.querySelector('.el-dialog .el-select__wrapper').click()`)
  await until(()=>evaluate(`(()=>{const e=[...document.querySelectorAll('.el-select-dropdown__item')].find(e=>e.textContent.trim()==='浏览器标准产品'&&e.getClientRects().length);if(!e)return false;e.click();return true})()`))
  await fill('设备名称','浏览器传感器修订'); await fill('设备标识','browser-device')
  await click('保存设备')
  await until(()=>evaluate(`document.querySelector('.el-dialog .el-descriptions__content')?.textContent`))
  const credentials=await evaluate(`Array.from(document.querySelectorAll('.el-dialog .el-descriptions__content')).map(e=>e.textContent.trim())`)
  const deviceCredential=['browser-device','',credentials[0],credentials[1]]
  await click('关闭')
  await until(()=>evaluate(`(()=>{const row=[...document.querySelectorAll('.el-table__row')].find(e=>e.textContent.includes('browser-device'));const button=[...(row?.querySelectorAll('button')||[])].find(e=>e.textContent.trim()==='连接详情');if(!button)return false;button.click();return true})()`))
  ...
  await click('接入测试')
  await until(()=>evaluate(`(()=>{const tab=[...document.querySelectorAll('[role=tab]')].find(e=>e.textContent.trim()==='测试设备');if(!tab)return false;tab.click();return true})()`))
  ...
  console.log('PASS: independent product and device registration, legacy Modbus API fixture, connection history, mobile drawer, legacy profile selection, listener sessions and recent devices, HTTP credential rejection and deduplication, report export without secrets, credential cleanup')
} finally {
