...
try {
  ...
  await fill('标准设备标识','browser-device');await fill('接入密钥',deviceCredential[2]);await fill('设备 Secret','invalid-secret')
  await click('发送新消息')
  await until(()=>evaluate(`document.querySelector('.standard-commissioning .el-alert')?.textContent.includes('AUTH_FAILED')`))
  await fill('设备 Secret',deviceCredential[3]);await click('发送新消息')
  ...
  console.log('PASS: real browser onboarding, Edge node registration, new product, test, preview, enable, connection history, mobile drawer, legacy profile selection, listener sessions and recent devices, HTTP credential rejection and deduplication, report export without secrets, credential cleanup')
} finally {
