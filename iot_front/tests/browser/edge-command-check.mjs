...
try {
  ...
  await click('设备接入')
  await until(()=>evaluate(`(()=>{const e=[...document.querySelectorAll('[role=tab]')].find(e=>e.textContent.trim()==='网络监听');if(!e)return false;e.click();return true})()`))
  ...
  await evaluate(`(()=>{const item=[...document.querySelectorAll('.el-form-item')].find(e=>e.querySelector('label')?.textContent.trim()==='协议包支持的命令结构化数据');const e=item.querySelector('textarea');e.value='{"type":"test"}';e.dispatchEvent(new Event('input',{bubbles:true}))})()`)
  await click('发送命令')
  ...
  await until(()=>evaluate(`(()=>{const e=[...document.querySelectorAll('[role=tab]')].find(e=>e.textContent.trim()==='接入实例');if(!e)return false;e.click();return true})()`))
  ...
  await click('下一步')
  await evaluate(`([...document.querySelectorAll('.el-radio')].find(e=>e.textContent.includes('网络长连接'))).click()`)
  await click('下一步')
  await evaluate(`document.querySelector('.el-dialog .el-switch').click()`)
  await choose('现场 边缘节点','现场节点')
