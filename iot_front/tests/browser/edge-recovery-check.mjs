...
try {
  ...
  await click('边缘节点')
  await click('运行与分配')
  await until(()=>evaluate(`document.body.textContent.includes('损坏隔离：1')`))
  assert.equal(await evaluate(`document.body.textContent.includes('损坏文件已保留在现场节点的 outbox 目录')`),true)
  const state=await evaluate(`fetch('/api/v1/edge-nodes/node/runtime',{headers:{Authorization:'Bearer '+localStorage.getItem('iot_token')}}).then(r=>r.json())`)
  assert.equal(state.heartbeat.corruptDepth,1)
  await call('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true})
  assert.equal(await evaluate(`!![...document.querySelectorAll('.el-dialog')].find(e=>e.textContent.includes('损坏隔离：1')&&e.getBoundingClientRect().width<=390)`),true)
  console.log('PASS: actual recovered node heartbeat, persistent corrupt quarantine count, operator guidance and narrow screen')
