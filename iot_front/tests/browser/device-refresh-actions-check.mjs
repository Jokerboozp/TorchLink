...
try{
 ...
 if(!groups.length||groups.some(g=>g.includes('下载制品')))throw Error('协议操作栏仍包含多余操作')
 const buttons=await evaluate(`[...document.querySelectorAll('.release-buttons button')].map(b=>({text:b.textContent.trim(),disabled:b.disabled,link:b.classList.contains('is-link'),column:!!b.closest('td.el-table-fixed-column--right'),border:getComputedStyle(b).borderTopColor,background:getComputedStyle(b).backgroundColor}))`)
 if(buttons.some(b=>b.border==='rgba(0, 0, 0, 0)'||b.background==='rgba(0, 0, 0, 0)'))throw Error('协议按钮被全局样式显示成文字链接')
 if(buttons.some(b=>b.disabled))throw Error('协议操作栏显示了不可用按钮')
 if(!buttons.some(b=>b.text==='解析测试'&&!b.link&&b.column))throw Error('可用的解析测试未显示在固定操作栏')
