...
try{ /* 执行当前语句并推进处理流程。 */
 ...
 const storage={iot_token:auth.accessToken,iot_tenant:tenant,iot_role:'admin',iot_user:user} /* 声明 storage。 */
 ...
 await evaluate(`document.querySelector('.menu-item[aria-label="设备管理"]').click()`);await delay(1200) /* 等待异步操作完成。 */
 await evaluate(`(()=>{window.__registryRequests=0;const original=window.fetch;window.fetch=(...args)=>{if(String(args[0]).includes('/device-registry'))window.__registryRequests++;return original(...args)};for(let i=0;i<20;i++)window.dispatchEvent(new CustomEvent('iot:realtime',{detail:{topic:'device.state'}}))})()`) /* 等待异步操作完成。 */
 await delay(300) /* 等待异步操作完成。 */
 if(await evaluate('window.__registryRequests')!==0)throw Error('实时消息触发整表重载') /* 判断条件并选择处理分支。 */
 if(!await evaluate(`document.body.innerText.includes('有新数据')`))throw Error('未提示新数据') /* 判断条件并选择处理分支。 */
 ...
 await writeFile(dir+'/screenshots/protocol-actions.png',Buffer.from((await call('Page.captureScreenshot',{format:'png'})).data,'base64')) /* 等待异步操作完成。 */
 await evaluate(`[...document.querySelectorAll('.release-buttons button')].find(b=>b.textContent.trim()==='解析测试'&&!b.disabled).click()`) /* 等待异步操作完成。 */
 await until(()=>evaluate(`[...document.querySelectorAll('.el-dialog')].some(e=>e.getClientRects().length)`)) /* 等待异步操作完成。 */
 console.log('通过：协议按钮位于操作栏，解析测试可打开') /* 执行当前语句并推进处理流程。 */
 if(errors.some(e=>e.error||e.path?.startsWith('/api/')))throw Error('页面请求或运行异常') /* 判断条件并选择处理分支。 */
