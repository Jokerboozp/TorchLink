...
try { /* 执行当前语句并推进处理流程。 */
  ...
  await until(()=>evaluate(`document.querySelector('.ui-drawer')?.textContent.includes('子设备（1）')`)) /* 等待异步操作完成。 */
  assert.ok(await evaluate(`document.querySelector('.ui-drawer').textContent.includes('sensor')`)) /* 验证实际结果符合预期。 */
  await click('查看子设备') /* 等待异步操作完成。 */
  await until(()=>evaluate(`document.querySelector('.ui-drawer')?.textContent.includes('所属主设备')`)) /* 等待异步操作完成。 */
  assert.ok(await evaluate(`document.querySelector('.ui-drawer').textContent.includes('主设备通信会话')`)) /* 验证实际结果符合预期。 */
  ...
  await until(()=>evaluate(`document.querySelector('.ui-drawer').getBoundingClientRect().width<=390`)) /* 等待异步操作完成。 */
  ...
  await evaluate(`document.querySelector('[aria-label="子设备产品"]').closest('.ui-select').querySelector('.n-base-selection').click()`) /* 等待异步操作完成。 */
