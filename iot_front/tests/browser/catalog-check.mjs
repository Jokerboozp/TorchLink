...
try {
  ...
  await click('协议管理'); await click('更多')
  await until(()=>evaluate(`(()=>{const e=[...document.querySelectorAll('[role=menuitem]')].find(e=>e.textContent.trim()==='协议目录');if(!e)return false;e.click();return true})()`))
  await until(()=>evaluate(`document.body.textContent.includes('目录示例')`))
  await click('安装并校验')
  await until(()=>evaluate(`document.querySelector('.el-message-box')?.textContent.includes('服务进程权限')`))
  await evaluate(`(()=>{const e=[...document.querySelectorAll('.el-message-box button')].find(e=>e.textContent.trim()==='安装并校验');if(!e)throw new Error('confirmation missing');e.click()})()`)
  await until(()=>evaluate(`document.body.textContent.includes('源码与样例校验通过')`))
  await until(()=>evaluate(`document.querySelector('.el-dialog')?.textContent.includes('已安装')`))
  await call('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true})
  assert.equal(await evaluate(`!![...document.querySelectorAll('button')].find(e=>e.textContent.trim()==='已安装'&&e.disabled)`),true)
  console.log('PASS: signed HTTPS catalog, explicit code execution confirmation, actual compile and samples, installed version and duplicate prevention')
} finally {
