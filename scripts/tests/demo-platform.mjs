...
async function req(path,{method='GET',body,device=false,raw=false,binary=false,authToken=token}={}){
 ...
 if(!response.ok){const text=await response.text().catch(()=>'');let reason='';try{const v=JSON.parse(text);reason=v.detail||v.error||v.title||''}catch{reason=text.slice(0,160)}const failure=new Error(`${method} ${path}: HTTP ${response.status}${reason?' · '+reason:''}`);failure.status=response.status;throw failure}
