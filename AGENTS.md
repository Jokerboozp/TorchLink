...
- Worker 是具有服务进程操作系统权限的子进程，超时和最小环境变量不等于强隔离沙箱。不要把逐报文 Worker 描述为常驻边缘采集。中心 TCP 主动连接与定时查询由接入网关配置（`DeviceAccessProfile`）调度，细节见 `docs/TCP_CHILD_DEVICE_ACCESS.md`；不把跨副本会话路由或任意连接事件回调视为已有能力。
