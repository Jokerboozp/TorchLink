...
- 项目按从零开发推进，Go 协议统一使用 `go-protocol-v2`，不维护旧 Worker 契约、兼容上传入口或解析回退。Go 函数模板和完整 Go 项目遵守同一契约。
...
- Worker 是具有服务进程操作系统权限的子进程，超时和最小环境变量不等于强隔离沙箱。常驻 Worker 只是复用解析进程，不要描述为常驻边缘采集。中心 TCP 主动连接与定时查询由接入网关配置（`DeviceAccessProfile`）调度，细节见 `docs/TCP_CHILD_DEVICE_ACCESS.md`；不把跨副本会话路由或任意连接事件回调视为已有能力。

## 7. AI、知识检索与 MCP

- Harness 为必装组件，所有使用模型的业务功能（告警自动与手动研判、巡检建议、运维报告、协议助手、规则草稿）都作为 Harness 工作流运行，不再直接调用 Provider；Provider 只负责模型管理页的连接测试、健康检查与配置同步。排查时分别核对 Harness 工作流、Provider 连接和进程加载的配置。
- 本地、在线和离线部署统一使用 DeepSeek API，用户填写 API Key；不下载或打包 Qwen 对话模型。Ollama 仅为知识库提供 `nomic-embed-text` 嵌入，离线安装不代表 AI 可断网使用。
...
- MQTT、HTTP、TCP、Modbus、Ollama、DeepSeek、OpenAI、JSON、操作系统及 CPU 架构等专业名称保留通用写法，必要时附中文说明；不以泛化中文替代具体技术名称。状态、按钮和业务说明继续用中文。
...
- [对外开放接口](docs/OPEN_API.md)：外部系统密钥、告警与设备消息上报、查询及智能问答。
