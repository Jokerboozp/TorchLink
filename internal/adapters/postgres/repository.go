...
func (r *Repository) SaveProtocolPackage(ctx context.Context, v model.ProtocolPackage) error {
	b, _ := json.Marshal(v)
	_, err := r.pool.Exec(ctx, saveProtocolPackageSQL, v.TenantID, v.ID, v.Status, v.ParserType, b)
	return err
}
...
func (r *Repository) SaveProductProtocolBinding(ctx context.Context, v model.ProductProtocolBinding) error {
	...
	_, err = r.pool.Exec(ctx, saveBindingSQL, v.TenantID, v.ProductID, v.ProtocolID, v.Version, b)
	return err
}
...
func insertOutbox(ctx context.Context, tx pgx.Tx, v model.OutboxEvent) error {
	_, err := tx.Exec(ctx, `INSERT INTO event_outbox(topic,event_key,payload) VALUES($1,$2,$3)`, v.Topic, v.Key, v.Payload)
	return err
}

// DrainOutbox locks one batch (concurrent relays skip it) and deletes the
// published rows in the same transaction.
func (r *Repository) DrainOutbox(ctx context.Context, limit int, publish func(model.OutboxEvent) error) (int, error) {
	tx, err := r.pool.Begin(ctx)
	if err != nil {
		return 0, err
	}
	defer tx.Rollback(ctx)
	rows, err := tx.Query(ctx, `SELECT seq,topic,event_key,payload FROM event_outbox ORDER BY seq LIMIT $1 FOR UPDATE SKIP LOCKED`, limit)
	if err != nil {
		return 0, err
	}
	events, err := pgx.CollectRows(rows, func(row pgx.CollectableRow) (model.OutboxEvent, error) {
		var v model.OutboxEvent
		return v, row.Scan(&v.Seq, &v.Topic, &v.Key, &v.Payload)
	})
	if err != nil {
		return 0, err
	}
	...
	for _, v := range events {
		if publishErr = publish(v); publishErr != nil {
			break
		}
		sent = append(sent, v.Seq)
	}
	if len(sent) > 0 {
		if _, err = tx.Exec(ctx, `DELETE FROM event_outbox WHERE seq=ANY($1)`, sent); err == nil {
			err = tx.Commit(ctx)
		}
		if err != nil {
			return 0, err
...
func (r *Repository) SaveAIAnalysis(ctx context.Context, v model.AIAnalysis) error {
	b, _ := json.Marshal(v)
	_, err := r.pool.Exec(ctx, `INSERT INTO alarm_ai_analysis(tenant_id,alarm_id,knowledge_scope,body) VALUES($1,$2,$3,$4) ON CONFLICT(tenant_id,alarm_id,knowledge_scope) DO UPDATE SET body=excluded.body,created_at=now()`, v.TenantID, v.AlarmID, v.KnowledgeScope, b)
	return err
}
func (r *Repository) GetAIAnalysis(ctx context.Context, tenant, id, knowledgeScope string) (model.AIAnalysis, error) {
	var v model.AIAnalysis
	var b []byte
	err := r.pool.QueryRow(ctx, `SELECT body FROM alarm_ai_analysis WHERE tenant_id=$1 AND alarm_id=$2 AND knowledge_scope=$3`, tenant, id, knowledgeScope).Scan(&b)
	if errors.Is(err, pgx.ErrNoRows) {
		return v, ErrNotFound
	}
	if err == nil {
...
func (r *Repository) CreateAlarmAnalysisJob(ctx context.Context, v model.AlarmAnalysisJob) (bool, error) {
	...
	defer tx.Rollback(ctx)
	// Keep only the newest job per alarm and scope; the partial unique index on
	// running jobs turns a concurrent start into a no-op.
	if _, err = tx.Exec(ctx, `DELETE FROM alarm_analysis_job WHERE tenant_id=$1 AND alarm_id=$2 AND knowledge_scope=$3 AND status<>'running'`, v.TenantID, v.AlarmID, v.KnowledgeScope); err != nil {
		return false, err
	}
	tag, err := tx.Exec(ctx, `INSERT INTO alarm_analysis_job(tenant_id,id,alarm_id,knowledge_scope,status,started_at,updated_at,body) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT DO NOTHING`, v.TenantID, v.ID, v.AlarmID, v.KnowledgeScope, v.Status, v.StartedAt, v.UpdatedAt, b)
	if err != nil {
		return false, err
	}
	return tag.RowsAffected() == 1, tx.Commit(ctx)
}
func (r *Repository) UpdateRunningAlarmAnalysisJob(ctx context.Context, v model.AlarmAnalysisJob) (bool, error) {
	b, _ := json.Marshal(v)
	tag, err := r.pool.Exec(ctx, `UPDATE alarm_analysis_job SET status=$3,updated_at=$4,body=$5 WHERE tenant_id=$1 AND id=$2 AND status='running'`, v.TenantID, v.ID, v.Status, v.UpdatedAt, b)
	return err == nil && tag.RowsAffected() == 1, err
}
func (r *Repository) LatestAlarmAnalysisJob(ctx context.Context, tenant, alarmID, knowledgeScope string) (model.AlarmAnalysisJob, error) {
	var v model.AlarmAnalysisJob
	var b []byte
	err := r.pool.QueryRow(ctx, `SELECT body FROM alarm_analysis_job WHERE tenant_id=$1 AND alarm_id=$2 AND knowledge_scope=$3 ORDER BY started_at DESC, updated_at DESC LIMIT 1`, tenant, alarmID, knowledgeScope).Scan(&b)
	if errors.Is(err, pgx.ErrNoRows) {
		return v, ErrNotFound
	}
	if err == nil {
...
func (r *Repository) LoadAIProviderConfig(ctx context.Context) (ports.AIPluginConfig, bool, error) {
	...
	return ports.AIPluginConfig{Provider: provider, BaseURL: values.BaseURL, Model: modelName, APIKey: values.APIKey, MaxTokens: values.MaxTokens}, true, nil
}
func (r *Repository) SaveAIProviderConfig(ctx context.Context, v ports.AIPluginConfig) error {
	raw, _ := json.Marshal(map[string]any{"baseUrl": v.BaseURL, "apiKey": v.APIKey, "maxTokens": v.MaxTokens})
