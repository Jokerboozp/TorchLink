...
func NewProviderRegistry() *ProviderRegistry {
	...
	r.register(providerFactory{
		info: ports.AIPluginInfo{ID: "deepseek", Name: "DeepSeek", Description: "DeepSeek 官方 OpenAI-compatible API。", DefaultBaseURL: "https://api.deepseek.com", DefaultModel: "deepseek-flash", RequiresAPIKey: true, Enabled: true, Capabilities: []string{"chat", "alarm-analysis", "rule-draft", "json-output"}},
		build: func(cfg ports.AIPluginConfig) (ports.AIClient, error) {
			if strings.TrimSpace(cfg.APIKey) == "" {
				return nil, fmt.Errorf("DeepSeek API Key is required")
			}
			return NewOpenAICompatible("deepseek", "DeepSeek", valueOr(cfg.BaseURL, "https://api.deepseek.com"), valueOr(cfg.Model, "deepseek-flash"), cfg.APIKey)
		},
	})
	r.register(providerFactory{
