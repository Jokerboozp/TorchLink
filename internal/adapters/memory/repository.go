...
func (r *Repository) ListRawIndexes(_ context.Context, f ports.RawFilter) ([]model.RawArchiveIndex, error) {
	...
	for _, v := range r.raw {
		if !matchesRawFilter(v, standards[key(v.TenantID, v.MessageID)], f) {
			continue
		}
		out = append(out, v)
	}
...
// Caller holds r.mu.
func (r *Repository) addOutbox(v model.OutboxEvent) {
	r.outboxSeq++
	v.Seq = r.outboxSeq
	r.outbox = append(r.outbox, v)
}

// DrainOutbox publishes without holding r.mu, because in-process bus handlers
// may call back into the repository.
func (r *Repository) DrainOutbox(_ context.Context, limit int, publish func(model.OutboxEvent) error) (int, error) {
	r.mu.RLock()
	batch := append([]model.OutboxEvent(nil), r.outbox[:min(limit, len(r.outbox))]...)
	r.mu.RUnlock()
	sent := map[int64]bool{}
	var err error
	for _, v := range batch {
	...
	r.mu.Lock()
	r.outbox = slices.DeleteFunc(r.outbox, func(v model.OutboxEvent) bool { return sent[v.Seq] })
	r.mu.Unlock()
	return len(sent), err
}
...
func (r *Repository) GetAIAnalysis(_ context.Context, tenant, id, knowledgeScope string) (model.AIAnalysis, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	v, ok := r.ai[key(tenant, id, knowledgeScope)]
	if !ok {
		return v, ErrNotFound
	}
	return v, nil
