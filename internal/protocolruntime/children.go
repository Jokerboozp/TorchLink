...
func (s *listenerSession) ingestChildren(p model.DeviceAccessProfile, parent model.ManagedDevice, source model.RawMessage, children []protocolworker.ChildFrame) error {
	...
	for index, child := range children {
		...
		if len(s.pending) > 0 && s.commandChild != nil && child.Address == s.commandChild.ChildAddress && product == s.commandChild.ProductID {
...
func (s *listenerSession) encodeCommand(ctx context.Context, p model.DeviceAccessProfile, child *model.ManagedDevice, command map[string]any) (protocolworker.Response, error) {
	...
	if child.Status != "ENABLED" || child.DeviceRole != "CHILD" || child.ConnectorProfileID != p.ID {
		return empty, errors.New("子设备不可控制或不属于接入实例")
	}
	allowed := false
	for _, mapping := range p.ChildProducts {
		if mapping.Type == child.ChildType && mapping.ProductID == child.ProductID {
			allowed = true
		}
	}
	if !allowed {
	...
	outer, err := s.invoke(ctx, p, protocolworker.Request{Operation: "encode", Command: map[string]any{"type": "child", "address": child.ChildAddress, "childType": child.ChildType, "payload": inner.Reply, "correlationId": inner.CorrelationID}})
	if err == nil {
