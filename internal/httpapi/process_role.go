...
func accessRoute(path string) bool {
	if strings.HasPrefix(path, "/api/v1/device-ingest/") || path == "/api/v1/device-mqtt/token" || path == "/api/v1/connectors" || path == "/api/v1/onboarding" || path == "/api/v1/onboarding/preflight" {
		return true
	}
