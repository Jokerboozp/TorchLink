...
func TestOnboardingBrowser(t *testing.T) { /* 定义 TestOnboardingBrowser 函数。 */
	...
	if cfg.MQTTWebSocketURL != "" && os.Getenv("IOT_TEST_MQTT_WEBSOCKET") != "" { /* 判断条件并选择处理分支。 */
		...
		jwt, e := auth.New(cfg.JWTSecret).IssueWithACL(username, "tenant", "service", nil, []auth.ACLRule{{Permission: "allow", Action: "subscribe", Topic: "/iot/up/#"}}, 2*time.Minute) /* 更新 e 的值。 */
