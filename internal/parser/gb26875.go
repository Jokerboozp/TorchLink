...
type GB26875Parser struct{} /* 定义 GB26875Parser 类型。 */

func (GB26875Parser) Name() string    { return "gb26875_dahua_parser" } /* 定义 Name 函数。 */
func (GB26875Parser) Version() string { return "1.0.0" }                /* 定义 Version 函数。 */
func (GB26875Parser) Match(m Meta) bool { /* 定义 Match 函数。 */
	p := strings.ToLower(m.Protocol)                                                                                          /* 更新 p 的值。 */
	return strings.EqualFold(m.PayloadFormat, "hex") && (strings.Contains(p, "gb26875") || strings.Contains(p, "dahua-fire")) /* 返回当前处理结果。 */
} /* 结束当前表达式或代码块。 */

type gb26875Frame struct { /* 定义 gb26875Frame 类型。 */
...
func (GB26875Parser) Parse(raw model.RawMessage) (*model.StandardMessage, error) { /* 定义 Parse 函数。 */
	...
	frame, err := decodeGB26875Frame(data) /* 更新 err 的值。 */
	if err != nil {                        /* 判断条件并选择处理分支。 */
		return nil, err /* 返回当前处理结果。 */
	} /* 结束当前表达式或代码块。 */
	msg := &model.StandardMessage{ /* 更新 msg 的值。 */
		...
		MessageType:  model.PropertyReport,                                                          /* 执行当前语句并推进处理流程。 */
...
func decodeGB26875Frame(data []byte) (gb26875Frame, error) { /* 定义 decodeGB26875Frame 函数。 */
	if len(data) < 2+gb26875ControlLength+1+2 { /* 判断条件并选择处理分支。 */
		return gb26875Frame{}, errors.New("GB26875 frame is too short") /* 返回当前处理结果。 */
	} /* 结束当前表达式或代码块。 */
	if data[0] != '@' || data[1] != '@' { /* 判断条件并选择处理分支。 */
		return gb26875Frame{}, errors.New("GB26875 start marker must be @@") /* 返回当前处理结果。 */
	} /* 结束当前表达式或代码块。 */
	if data[len(data)-2] != '#' || data[len(data)-1] != '#' { /* 判断条件并选择处理分支。 */
		return gb26875Frame{}, errors.New("GB26875 end marker must be ##") /* 返回当前处理结果。 */
	} /* 结束当前表达式或代码块。 */
	dataLength := int(binary.LittleEndian.Uint16(data[24:26])) /* 更新 dataLength 的值。 */
	if dataLength > gb26875MaxDataLength {                     /* 判断条件并选择处理分支。 */
		return gb26875Frame{}, fmt.Errorf("GB26875 application data length %d exceeds %d", dataLength, gb26875MaxDataLength) /* 返回当前处理结果。 */
	} /* 结束当前表达式或代码块。 */
	wantLength := 2 + gb26875ControlLength + dataLength + 1 + 2 /* 更新 wantLength 的值。 */
	if len(data) != wantLength {                                /* 判断条件并选择处理分支。 */
		return gb26875Frame{}, fmt.Errorf("GB26875 frame length mismatch: header says %d application bytes, frame has %d bytes", dataLength, len(data)) /* 返回当前处理结果。 */
	} /* 结束当前表达式或代码块。 */
	var sum byte                                    /* 声明 sum。 */
	...
	if got := data[27+dataLength]; got != sum { /* 判断条件并选择处理分支。 */
		return gb26875Frame{}, fmt.Errorf("GB26875 checksum mismatch: got %02X, want %02X", got, sum) /* 返回当前处理结果。 */
	} /* 结束当前表达式或代码块。 */
	return gb26875Frame{ /* 返回当前处理结果。 */
...
func applyGB26875Data(msg *model.StandardMessage, frame gb26875Frame) error { /* 定义 applyGB26875Data 函数。 */
	...
	if frame.Command == 0x03 && len(frame.Data) == 0 { /* 判断条件并选择处理分支。 */
		msg.MessageType = model.CommandReply      /* 更新 msg.MessageType 的值。 */
	...
	if frame.Command == 0x00 && typeFlag == 0x00 {        /* 判断条件并选择处理分支。 */
		...
		msg.MessageType = model.StateChange                                                                                                                                              /* 更新 msg.MessageType 的值。 */
	...
	if frame.Command == 0x01 && typeFlag == 0x5A { /* 判断条件并选择处理分支。 */
		...
		if isGB26875PlatformAddress(frame.Source) { /* 判断条件并选择处理分支。 */
			...
			msg.MessageType = model.EventReport                                        /* 更新 msg.MessageType 的值。 */
...
func applyGB26875ComponentStatus(msg *model.StandardMessage, data []byte, count int) error { /* 定义 applyGB26875ComponentStatus 函数。 */
	...
	msg.Event = map[string]any{"type": "COMPONENT_STATUS", "alarm": anyAlarm, "fault": anyFault, "objects": objects} /* 更新 msg.Event 的值。 */
	if anyAlarm || anyFault {                                                                                        /* 判断条件并选择处理分支。 */
		msg.MessageType = model.AlarmReport /* 更新 msg.MessageType 的值。 */
	} else { /* 结束当前表达式或代码块。 */
		msg.MessageType = model.StateChange /* 更新 msg.MessageType 的值。 */
	} /* 结束当前表达式或代码块。 */
	return nil /* 返回当前处理结果。 */
...
func decodeGB26875Time(data []byte) int64 { /* 定义 decodeGB26875Time 函数。 */
	...
	t := time.Date(year, time.Month(values[4]), values[3], values[2], values[1], values[0], 0, time.Local)                                            /* 更新 t 的值。 */
