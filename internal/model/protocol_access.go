...
func ExistingProtocolChild(old, next ManagedDevice) error {
	if old.TenantID != next.TenantID || old.ProductID != next.ProductID || old.GatewayID != next.GatewayID || old.DeviceRole != "CHILD" || old.Status != "ENABLED" || old.ChildAddress != next.ChildAddress || old.ChildType != next.ChildType {
