...
**炬联 TorchLink** 是面向消防设备联网、运行监测与辅助运维的物联网平台。平台以设备数据为基础，将多协议接入、协议源码与可信分发、原始报文归档、标准化解析、规则告警、知识检索和 AI 辅助分析连接起来，帮助实施人员接好设备、帮助值守人员理解告警、帮助开发人员追溯和修正协议问题。
...
| 主要交付形态 | 源码部署、Docker 在线部署、离线安装包；可选独立接入网关 |

**功能核对日期：2026-09-10。** 功能说明依据当前仓库源码与配置，包括中心接入、可信目录和企业私有协议市场。外部平台对比沿用 **2026-09-09** 查阅的官方资料，商业产品说明与社区版源码分开表述。源码具备相应实现不代表目标环境已验收；本文不作为性能测试报告或生产验收证明。
...
- **项目维护团队**：管理模型配置、知识文档、数据备份以及接入服务的运行状态。
...
- 产品物模型与元数据，声明属性类型、可写性、事件和功能，为数据解释、规则和设备命令提供依据。
...
| Modbus TCP / RTU over TCP | 点表配置与导入、读取预览、周期采集、原始响应解析；支持串口服务器 RTU 透传 | 须配置正确的站号、寄存器、类型、字节序和网络访问范围 |
| TCP / UDP 自定义协议 | TCP 双向连接 / UDP 监听与 Go 协议包协同完成分帧、设备识别、应答和下行 | 具体厂商格式由协议包实现；下行需要协议支持及可用会话 |
...
源码入口：[协议运行时](internal/protocolruntime/)。接入细节见[设备接入说明](docs/UNIFIED_DEVICE_ONBOARDING.md)；现场入口移除范围见[说明](docs/EDGE_REMOVAL.md)。
...
4. **运行样例**：在服务器实际运行本机制品的解析及接入、编码样例；其他目标仅生成制品，交叉编译不等于该平台样例通过。
...
**实现边界**：Worker 当前按调用启动子进程，存在进程启动开销。超时和最小环境变量限制不构成强隔离沙箱；该入口面向可信协议开发者。中心运行时已支持 TCP 主动连接和配置式查询调度，Worker 仍只负责单次协议调用；任意定时任务和常驻协议进程不属于该契约。详见 [TCP 与主子设备接入](docs/TCP_CHILD_DEVICE_ACCESS.md)。
...
对于已支持下行的 MQTT 或自定义 TCP/UDP 协议，平台提供设备命令接口、命令记录和回执状态。现场节点命令队列已移除，中心命令仍检查当前连接和协议能力。
...
平台提供摄像头信息的人工登记、维护和设备关联；单个摄像头最多关联一个设备，设备可关联多个摄像头。外部视频告警事件仍可接入并关联业务告警。视频由外部平台提供，平台不拉流或预览；原现场 ONVIF 发现与节点目录导入随边缘模块移除。

源码入口：[摄像头页面](iot_front/src/views/CameraMappingsView.vue)、[视频接口](internal/httpapi/server.go)。
...
默认单进程运行；可按需拆分 API 与 Access Gateway，使用共享 PostgreSQL、Kafka 和协议制品目录。Gateway 负责通信、认证及原文接入，API 负责解析、业务与管理。执行租约和请求路由支持实例间互斥执行及接管，不迁移已有 TCP 会话。

现场 Agent、节点管理和程序升级已移除。MQTT 可靠接收依赖的磁盘队列保留为公共组件，旧节点任务不会在中心自动执行。

源码入口：[进程装配](internal/platformapp/)、[角色路由](internal/httpapi/process_role.go)、[执行协调](internal/protocolruntime/coordinator.go)、[持久队列](internal/durablequeue/)。配置见 [独立接入进程](docs/EDGE_AND_GATEWAY.md)和[移除说明](docs/EDGE_REMOVAL.md)。
...
| 视频能力 | 摄像头元数据、设备关联和外部事件 | 官网 AI 产品覆盖视频接入及分析流程。[AI](https://www.jetlinks.cn/ai) | 项目需要自身管理视频与视觉分析时，炬联当前范围明显不足 |
| 现场与边缘 | 当前移除现场 Agent；保留中心采集、Go 协议监听及执行协调 | Edge 产品覆盖采集、云边协同、热更新、自治与边端 AI。[Edge](https://www.jetlinks.cn/edge) | 按协议、升级策略和现场边界比较，炬联不含边缘 AI |
...
中心通过可达网络读取 Modbus TCP 点位，完成原文归档、标准化与业务处理。平台不再提供现场 Agent 或 RS485 采集入口；现场不可达时需先解决网络或使用设备自身的上报能力。
...
| 视频 | 摄像头元数据、设备关联和外部事件 | 平台内视频播放、录像、完整国标安全与级联能力、内置视觉算法 |
| 独立 Gateway | 中心进程拆分、租约路由、协议监听与采集 | 不提供现场 Agent、节点管理、边缘升级或跨节点无损会话迁移 |
...
本文沿 API 路由、核心逻辑、存储适配器和前端入口核对功能，重点检查现场采集、协议制品、程序升级、主子设备关联和组织分发。专题文档保留具体契约，测试和阶段记录说明各自环境及日期；历史通过记录不代表当前环境已重新验收。
...
| 独立 Gateway | [实现说明](docs/EDGE_AND_GATEWAY.md)、[阶段记录](docs/DEVICE_ACCESS_REFACTOR_PROGRESS.md) |
...
| 升级与分发 | [可信目录](docs/PROTOCOL_CATALOG.md)、[组织市场](docs/PRIVATE_PROTOCOL_MARKET.md) |
...
**炬联 TorchLink 是面向消防设备联网与辅助运维的物联网平台，集设备接入、协议分发、报文追溯、规则告警与 AI 知识辅助于一体，支持私有化与离线交付。**

### 完整版

炬联 TorchLink 面向消防系统集成商、项目运维团队与企业信息化部门，提供从设备接入到告警处理的数据链路。平台采用 Go API 与 Vue 管理端，支持标准 MQTT/HTTP 上报、Modbus 点表采集，以及通过独立 Go 源码包扩展厂商协议。原始报文、协议版本和解析结果关联保存，可用于查询、下载、回放与差异分析。平台同时提供主子设备关联、规则告警、摄像头元数据与设备关联、知识库、AI 告警研判和智能巡检，并支持中心接入网关及组织内可信协议分发。在线与离线部署工具为项目交付提供统一入口。炬联重点帮助团队接好设备、追溯数据、理解告警，并持续维护适合自身现场的协议与运维知识。
