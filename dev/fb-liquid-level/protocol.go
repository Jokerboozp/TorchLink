...
func Protocol() Definition {
	return Definition{
		Name: "富贝防爆液位传感器", Version: "1.0.0", Transport: "TCP",
		Decode: decodeSensor, Ingress: ingressSensor, Encode: encodeSensor,
		Samples: []Sample{
			{Name: "实时液位", Data: report(0x0000, []byte{0, 1, 0, 80, 0, 10, 0, 0, 4, 210, 0, 20, 0, 5, 0, 0, 1, 244, 0, 0, 11, 184, 0, 1, 0, 0}), Want: properties(map[string]any{"levelMeters": 12.34, "batteryLevel": 80, "uploadInterval": 10, "signalStrength": 20, "collectionInterval": 5, "alarmLowerLimit": 5.0, "alarmUpperLimit": 30.0, "alarmEnabled": true, "offset": 0, "partType": "液位传感器"})},
			{Name: "报警", Data: report(0x0016, []byte{0, 1}), Context: Context{Now: 1789000000000}, Want: Message{MessageType: "STATE_CHANGE", Properties: map[string]any{"alarm": true, "partType": "液位报警"}, Event: map[string]any{"components": []map[string]any{{"id": "sensor", "name": "液位传感器", "alarms": map[string]bool{"LEVEL_ALARM": true}}}}}},
		},
...
func decodeSensor(data []byte, ctx Context) (Message, error) {
	...
	case 0x16:
		...
		return Message{MessageType: "STATE_CHANGE", Properties: map[string]any{"alarm": active, "partType": "液位报警"}, Event: map[string]any{"components": []map[string]any{{"id": "sensor", "name": "液位传感器", "alarms": map[string]bool{"LEVEL_ALARM": active}}}}}, nil
	case 0x19:
		if len(body) != 10 {
			return Message{}, errors.New("历史数据须为 10 字节")
		}
		return properties(map[string]any{"partType": "液位传感器", "historicalLevelMeters": float64(int32(binary.BigEndian.Uint32(body[6:10]))) / 100, "historyTimestamp": int64(binary.BigEndian.Uint32(body[:4])) * 1000}), nil
	default:
