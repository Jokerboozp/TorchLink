...
func Protocol() Definition { /* 定义 Protocol 函数。 */
	return Definition{ /* 返回当前处理结果。 */
		Name: "富贝防爆液压传感器", Version: "1.0.0", Transport: "TCP", /* 执行当前语句并推进处理流程。 */
		Decode: decodeSensor, Ingress: ingressSensor, Encode: encodeSensor, /* 执行当前语句并推进处理流程。 */
		Samples: []Sample{ /* 执行当前语句并推进处理流程。 */
			{Name: "实时压力", Data: report(0x0000, []byte{0, 1, 0, 80, 0, 10, 0, 0, 4, 210, 0, 20, 0, 5, 0, 0, 1, 244, 0, 0, 11, 184, 0, 1, 0, 0}), Want: properties(map[string]any{"pressureMPa": 12.34, "batteryLevel": 80, "uploadInterval": 10, "signalStrength": 20, "collectionInterval": 5, "alarmLowerLimit": 5.0, "alarmUpperLimit": 30.0, "alarmEnabled": true, "offset": 0, "partType": "液压传感器"})}, /* 执行当前语句并推进处理流程。 */
			{Name: "报警", Data: report(0x0016, []byte{0, 1}), Context: Context{Now: 1789000000000}, Want: Message{MessageType: "STATE_CHANGE", Properties: map[string]any{"alarm": true, "partType": "液压报警"}, Event: map[string]any{"components": []map[string]any{{"id": "sensor", "name": "液压传感器", "alarms": map[string]bool{"PRESSURE_ALARM": true}}}}}},                                                 /* 执行当前语句并推进处理流程。 */
		}, /* 结束当前表达式或代码块。 */
...
func decodeSensor(data []byte, ctx Context) (Message, error) { /* 定义 decodeSensor 函数。 */
	...
	case 0x16: /* 处理当前分支。 */
		...
		return Message{MessageType: "STATE_CHANGE", Properties: map[string]any{"alarm": active, "partType": "液压报警"}, Event: map[string]any{"components": []map[string]any{{"id": "sensor", "name": "液压传感器", "alarms": map[string]bool{"PRESSURE_ALARM": active}}}}}, nil /* 返回当前处理结果。 */
	case 0x19: /* 处理当前分支。 */
		if len(body) != 10 { /* 判断条件并选择处理分支。 */
			return Message{}, errors.New("历史数据须为 10 字节") /* 返回当前处理结果。 */
		} /* 结束当前表达式或代码块。 */
		return properties(map[string]any{"partType": "液压传感器", "historicalPressureMPa": float64(int32(binary.BigEndian.Uint32(body[6:10]))) / 100, "historyTimestamp": int64(binary.BigEndian.Uint32(body[:4])) * 1000}), nil /* 返回当前处理结果。 */
	default: /* 处理当前分支。 */
