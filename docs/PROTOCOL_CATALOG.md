...
目录也可以签名发布 `kind=edge-agent` 的平台专属程序制品，由现场独立启动器下载和切换，见 [Edge 程序升级](EDGE_PROGRAM_UPGRADES.md)。这类条目不会出现在协议源码安装列表；源码条目的 kind 可省略或设为 `protocol-source`。
...
这是平台的可信私有协议分发目录，未连接第三方公共市场。支付、评论、公共账号和跨组织审核平台仍未实现。签名证明来源与内容一致性，不保证代码安全；Worker 是具有 API 服务账户权限的子进程，管理员须信任签名发布方。
...
`caFile` 可为私有 CA 的本机 PEM 路径；留空使用系统信任。HTTPS 始终校验证书，不允许明文 HTTP、URL 凭据、查询参数或重定向。所有源码地址必须与目录地址同一 HTTPS origin。密钥轮换先加入新公钥，发布新签名目录后再移除旧公钥；下一次目录请求即重新读取信任配置。

Compose 已传入该变量。容器内路径须位于已有挂载中，例如 `/app/data/catalog-policy.json`；私有 CA 同样须可读。独立 API/Gateway 模式只需 API 读取目录，Edge 获取的仍是已分配且发布的既有协议制品。离线环境可使用局域网 HTTPS 目录，或沿用已有本地 ZIP 上传。
...
未实现持续后台自动升级、商业市场或跨组织审核。目录源码可在 `protocol.json` 中声明 `targetPlatforms`，安装复用源码多平台编译；发布端实际跑样例，异构 Edge 在本机试跑成功后才启用。目录签名不能替代这些执行校验，详见 [Go 协议包](GO_PROTOCOL_PACKAGES.md#异构-edge-制品)。
