...
消费平台不自动在后台安装更新，公共商业市场未实现。组织内审核发布与组织之间的授权分发见企业私有协议市场说明。目录源码可在 `protocol.json` 中声明 `targetPlatforms`，安装复用源码多平台编译；发布端实际跑样例，异构 Edge 在本机试跑成功后才启用。目录签名不能替代这些执行校验，详见 [Go 协议包](GO_PROTOCOL_PACKAGES.md#异构-edge-制品)。
