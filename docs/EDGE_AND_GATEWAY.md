...
损坏 JSON、超大记录或身份摘要不匹配的补传文件自动改名为 `.json.corrupt`，保留原始字节并计入队列容量，随后继续处理正常记录。心跳 `corruptDepth` 与“运行与分配”的“损坏隔离”显示待处置数量。`--retry-rejected` 不会重新排入这些文件；请先备份隔离文件并检查磁盘、原始数据与设备重传能力，再人工处置，不将损坏记录当作已上传。读取权限或文件系统 I/O 错误仍明确失败，不擅自移动不可读数据。
