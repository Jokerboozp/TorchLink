...
| 自定义 TCP 监听 | `internal/protocolruntime/listeners.go`：每个 Profile 最多 `IOT_PROTOCOL_LISTENER_MAX_SESSIONS`（默认 1024）个会话，超出的连接被关闭、计入 `protocol_listener_rejected_total` 并每 10 秒至多记一次警告；每进程 32 个 Worker 操作槽 |
