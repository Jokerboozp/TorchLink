...
“接入与测试”统一提供两个页内标签：“设备接入”查看已登记设备的连接参数和报文示例；“测试设备”保留 MQTT / HTTP 标准设备联调、测试设备准备、报文模板、快捷发送和结果查看。原接入指南中的内部报文联调弹窗已移除，模拟发送集中在“测试设备”；MQTT / HTTP 标准设备联调用于验证真实认证和连接。切换标签保留本次测试状态，离开页面后释放连接。原 `testDevice` 导航目标会打开“接入与测试”的“测试设备”标签。
