...
现场 Agent、节点登记与心跳、现场任务、程序升级、RTU 串口驱动、OPC UA / SNMP / BACnet 现场入口、ONVIF 发现及节点视频目录导入已移除。相关路由不再注册；协议目录、组织发布和远程分发入口也已移除，不再通过签名目录安装或升级程序。

`DeviceAccessProfile.edgeNodeId` 仅用于识别旧配置。非空配置不会在中心启动采集、监听、注册或下行，也不能重新保存为有效实例。需要改成中心接入时，先确认设备网络可达和产品协议，再到「接入网关」重新配置连接；旧串口任务不能仅清空节点标识后继续运行。外部已部署 Agent 和运行目录不会被平台自动停止或删除，需由部署者处理。
...
当前账户权限增加用户级设备范围。历史未设置范围的普通用户默认无设备，内置管理员仍保留当前租户维护入口。升级前签发的普通用户 MQTT 令牌需要撤销或等待最长15分钟过期；新版本普通用户改用受认证的 HTTP 消息接口。完整步骤见 [部署升级](DEPLOYMENT.md#用户权限升级)，范围与会话规则见 [用户权限](USER_ACCESS_CONTROL.md)。
