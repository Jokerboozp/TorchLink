...
管理员打开独立的“AI Provider”菜单后，可以选择“本地 Ollama”“DeepSeek API”或“OpenAI 兼容 API”，填写服务地址和模型名称，点击“测试并应用”。平台会先检查模型服务，再同时更新告警研判、AI 对话、规则草稿、报告和 Harness 工作流；正在运行的工作流结束后即可使用新配置，无需重启 API。Ollama 地址填写模型服务的根地址（例如 `http://192.168.24.133:11434`），若误填 `/v1` 平台会自动归一化；API Provider 填写兼容 Chat Completions 的根地址。AI Provider 页面还会列出所有共用当前 Provider 的 AI 业务入口。
