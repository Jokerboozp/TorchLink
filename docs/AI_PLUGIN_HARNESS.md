...
智能巡检任务的进度和报告保存在数据库表 `health_inspection_job`（内存仓储模式下保存在进程内），API 重启后和多个 API 副本之间都能读取同一任务；每个租户同时最多一个运行中的任务，由数据库唯一索引保证。巡检本身仍在发起任务的进程内执行：运行中的任务约每 0.5 秒更新一次心跳，超过 30 秒未更新即视为该进程已停止，读取时标记为“已中断”，可重新开始。巡检报告 PDF 复用 10 分钟内最近一次完成的报告，任意副本均可下载；报告过期后同一租户的并发下载只触发一次重新巡检。PDF 按严重程度逐台列出前 2000 台设备，其余只注明数量，完整清单在巡检页面查看。每个 API 进程同时最多渲染 2 份 PDF，等待超过 30 秒返回 429；同一份报告的 PDF 渲染后在进程内缓存，重复下载不再渲染。历史任务记录目前不自动清理。

手动告警研判任务同样保存在数据库表 `alarm_analysis_job`（内存仓储模式下保存在进程内）：同一告警、同一知识范围同时最多一个运行中的任务，由唯一索引保证；运行中约每 2 秒更新一次心跳，超过 30 秒未更新读取时标记为“已中断”，可重新发起。每个告警和知识范围只保留最新一次任务，新任务创建时替换已结束的旧任务；研判结果本身另存于 `alarm_ai_analysis`。
