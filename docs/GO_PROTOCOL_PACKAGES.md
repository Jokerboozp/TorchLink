...
选择绑定产品并发布后，新报文使用新版本；TCP/UDP 仍需配置接入网关。TCP 可选择设备连接平台或平台连接设备，UDP 使用监听端口；按所选方向配置实际地址和端口。这项简化不猜测设备地址、凭据、端口或厂家协议含义。
...
上传时表单非空值覆盖元数据；`id` 必须与 URL 协议标识一致。`TCP` / `UDP` 只支持对应网络，`TCP_UDP` 支持两种网络实例。每个接入网关固定一个租户和产品；同一网络监听端口只允许一个启用实例，TCP 主动连接不占用监听端口。新增版本须继续支持产品上所有已启用实例的网络。
...
协议生成和源码上传都在「协议管理」，连接配置在「接入网关」，模拟报文验证在「接入测试」。协议列表按协议条目分页，版本显示在版本区域；操作栏仅显示当前版本可用的解析测试、发布和源码下载操作。协议制品下载仍可通过 API 使用，管理页面不再展示。Go 源码版本先通过源码样例校验，真实设备接入需另行联调。
