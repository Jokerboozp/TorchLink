...
选择绑定产品并发布后，新报文使用新版本；TCP/UDP 仍需配置平台连接配置。TCP 可选择设备连接平台或平台连接设备，UDP 使用监听端口；按所选方向配置实际地址和端口。这项简化不猜测设备地址、凭据、端口或厂家协议含义。
...
平台连接配置配置属于全租户功能；普通用户需全部设备范围、设备管理及平台连接配置菜单和对应操作权限。源码/制品下载需协议管理菜单及各自下载权限；设备命令还需设备范围授权。下载 API：
...
协议生成和源码上传在「设备通信协议」，连接配置在「平台连接配置」，模拟报文验证在「模拟设备测试」。现场设备从「设备管理 → 接入设备」开始。协议列表按协议条目分页，版本显示在版本区域；操作栏仅显示当前版本可用的解析测试、发布和源码下载操作。协议制品下载仍可通过 API 使用，管理页面不再展示。Go 源码版本先通过源码样例校验，真实设备接入需另行联调。
