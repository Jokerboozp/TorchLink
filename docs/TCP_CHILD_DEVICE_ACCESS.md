...
平台提供两种 Go 协议 TCP 连接方向，在“平台连接配置 → 新建平台连接配置”维护。新增现场设备从“设备管理 → 接入设备”开始：
...
- **平台连接设备**：设置 `connectionMode=dial`，填写设备/串口服务器地址、端口及预配置的设备标识。平台保持连接，失败退避 1～30 秒重连；目标解析和 IP 校验复用 `IOT_MODBUS_ALLOWED_CIDRS` 的出站网络策略。先在设备管理登记设备，再保存主动连接实例。原子接入 API 仍可在同次请求中创建设备。
...
- 原始 RTU 字节透明转发：已有 **Modbus RTU 串口透传 TCP**（`MODBUS_RTU_TCP`）实例在平台连接配置页维护。实例保存 `mode=poll`、`network=tcp`、`wireFormat=rtu_over_tcp`，使用 `MODBUS_RTU` 版本化点表解析器。平台主动连接目标，发送带 CRC 的 RTU 查询，处理分片、站号、功能码、字节数和 CRC；归档保留原始 RTU 帧，不把转换后的 MBAP 帧冒充原文。
...
3. 在主设备平台连接配置配置 `childProducts`：`type` 是主协议从报文识别出的子设备类型，`productId` 是相应子设备产品。配置页实时显示该产品的协议及版本；产品未启用、未绑定协议或协议格式不兼容会被服务端拒绝。
...
主设备完整帧先归档，发现信息保存在 `metadata.children`；子设备原文单独进入 Raw→Parser→存储/告警链路。子原文固定子协议版本，记录 `parentRawMessageId` 和外层协议版本。登记或子原文接收失败时不发送本帧成功应答。未知类型的发现信息仍可从主设备原文查看，错误显示在平台连接配置；配置完成后由设备重新上报处理，不会猜测产品。
...
设备管理分为「独立设备、主设备、子设备」三个标签，并支持设备类型筛选。主设备“连接详情”展示分页子设备列表、产品/协议、最近上报及状态。只有分别授权后，用户才能进入关联主设备或子设备详情；主设备授权不会自动覆盖其子设备。全部设备范围用户可查看父连接，指定设备用户不展示共享平台连接配置和未授权父设备信息。
