...
平台提供两种 Go 协议 TCP 连接方向，在“接入实例 → 新建实例”配置：

- **设备连接平台**：`mode=listener`、`network=tcp`，`connectionMode=listen`（旧配置留空等价），填写平台监听 IP 和端口。每个实例绑定一个主设备产品，可按协议识别多个主设备。
- **平台连接设备**：同样使用 Go 协议接入实例，设置 `connectionMode=dial`，填写设备/串口服务器地址、端口及预配置的设备标识。平台保持连接，失败退避 1～30 秒重连；目标解析和 IP 校验复用 `IOT_MODBUS_ALLOWED_CIDRS` 的出站网络策略。先在设备管理登记设备，再保存主动连接实例。原子接入 API 仍可在同次请求中创建设备。
...
- 原始 RTU 字节透明转发：已有 **Modbus RTU 串口透传 TCP**（`MODBUS_RTU_TCP`）实例在接入实例页维护。实例保存 `mode=poll`、`network=tcp`、`wireFormat=rtu_over_tcp`，使用 `MODBUS_RTU` 版本化点表解析器。平台主动连接目标，发送带 CRC 的 RTU 查询，处理分片、站号、功能码、字节数和 CRC；归档保留原始 RTU 帧，不把转换后的 MBAP 帧冒充原文。
