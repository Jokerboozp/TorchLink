...
持久接收和设备上报与浏览器提醒是两条链路。所有用户从受认证的 `GET /api/v1/events` 获取授权设备的状态和活动告警，每3秒重新校验账户、角色及设备范围；管理员另用 MQTT 接收解析事件和规则界面联动。初次快照不重播历史报警，后续新增或更新才触发提醒，退出后的迟到响应会被丢弃。

部件告警按所属控制器的设备 ID 授权；主子设备台账则分别授权。列表、详情、总览和提醒使用同一设备范围，详见 [用户权限](USER_ACCESS_CONTROL.md)。本文的磁盘队列和 PUBACK 保障不表示浏览器一定在线或已经展示了提醒。
