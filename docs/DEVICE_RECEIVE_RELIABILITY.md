...
本文描述当前实现及使用边界；测试记录见 `DEVICE_ACCESS_REFACTOR_PROGRESS.md`。
...
不可解析的磁盘记录保留为 `.corrupt`；业务明确拒收的记录保留为 `.rejected`，都计入容量。未知错误默认可重试；数据库读取失败不能冒充设备认证失败而永久隔离。隔离原文仅供受信任运维人员检查，不能作为有效标准消息发布。目前没有新增队列管理页面；不得直接删除文件来宣称补传完成。
...
- `IOT_DATA_DIR` 必须挂载持久磁盘。接收队列是本机持久化，不是跨主机复制；主机损坏后的接管需要恢复原数据盘。本轮未改变共享存储或自动恢复主机的方案。
