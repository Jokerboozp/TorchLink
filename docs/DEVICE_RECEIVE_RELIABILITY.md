...
队列复用 Edge 的文件锁、原子写入和隔离机制，8 个固定分片，默认总上限 512 MiB / 10000 项，均分到各分片；热点主题可能先达到分片上限。主题和原文字节的摘要用于接收队列去重，不替代业务消息 ID。满容量、写入错误或损坏隔离导致无法确认时，不 ACK，并通过重连请求 Broker 重投；没有启动无界 goroutine 或无限内存队列。
